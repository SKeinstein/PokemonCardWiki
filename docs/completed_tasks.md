...
- [x] GROUP_ORDER / SECTION_MAP 更新（`>にげられない>ワザ` 削除、`>エネルギー` 追加）
- [x] `data/card_tags.json` 再ビルド確認
- [x] `group_categories_proposal.md` §B-8-9節・§B-8-10節・サマリー表更新

### Phase 37-5: ポケモンのどうぐ>参照 → カード種別参照>ポケモンのどうぐ 統合・§B-30廃止 ✅ (2026-05-17)
- [x] `tag_cards.mjs` §B-30ルール（`ポケモンのどうぐ`/`ポケモンのどうぐ>参照`）削除、§B-28内に `カード種別参照>ポケモンのどうぐ` 新設（同一条件）
- [x] GROUP_ORDER から `ポケモンのどうぐ`/`ポケモンのどうぐ>参照` 削除、`カード種別参照>ポケモンのどうぐ` を §B-28ブロックに追加
- [x] SECTION_MAP 同期（§B-30エントリ削除 / `カード種別参照>ポケモンのどうぐ: §B-28` 追加）
- [x] `data/card_tags.json` 再ビルド: `カード種別参照>ポケモンのどうぐ` 14件新設 / `ポケモンのどうぐ`・`ポケモンのどうぐ>参照` 0件確認
- [x] `group_categories_proposal.md` §B-30節削除・§B-28テーブル更新（ポケモンのどうぐ行追加14件/旧行廃止）・サマリー表 §B-30行削除・タグ一覧更新
- [x] `docs/tagging_stats.md` §B-30→§B-28>ポケモンのどうぐ 反映（再ビルドで自動更新）

### Phase 37-3: フィールド干渉>エネルギーつけかえ を相手場限定に絞込 ✅ (2026-05-17)
- [x] `フィールド干渉>エネルギーつけかえ` の条件に `/相手の.{0,15}ポケモンについているエネルギー.{1,30}つけ替える/` を必須化
- [x] 自分の場内つけかえ21名（エネルギーつけかえ/メガフシギバナex等）はエネ加速>つけかえ（§5-E）のみにタグ移行
- [x] 残存: ゲンガーex/ロケット団のサンダー/リグレー/ニャオニクス（4名7件）のみ
- [x] `data/card_tags.json` 再ビルド: `フィールド干渉>エネルギーつけかえ` 103件→7件、`フィールド干渉` 229件→133件
- [x] `group_categories_proposal.md` §B-6-1説明文・サンプル表・サマリー件数更新

### Phase 37-2: カード種別参照>スタジアム誤分類修正 + 条件ダメージ>エネルギー消費：手札 新設 ✅ (2026-05-17)
- [x] `カード種別参照>スタジアム` の過剰マッチ条件「`トラッシュできないなら、このワザは失敗`」を「`スタジアムをトラッシュ〜このワザは失敗`」型に限定修正
- [x] 手札エネトラッシュ系5件（ジュナイパー・カミツオロチ×2・ラランテス・ソウブレイズ）および誤タグ1件（メルメタル: 自己どうぐトラッシュ）を `カード種別参照>スタジアム` から除外
- [x] `条件ダメージ>エネルギー消費：手札`（§B-17-9）新設: 7件（カミツオロチ×2/ソウブレイズ/ラランテス/ジュナイパー/ヒートバーナー×2）
- [x] `data/card_tags.json` 再ビルド: `カード種別参照>スタジアム` 22件→16件
- [x] `group_categories_proposal.md` §B-17-9新設・§B-28スタジアム件数更新・各サマリー表更新

### Phase 35-9: 手札干渉パターン拡充 ✅ (2026-05-17)
- [x] `手札干渉>トラッシュ` に「相手は相手自身の手札を〜トラッシュ」型パターン追加: 30件→43件（+13: エンニュート/クセロシキのたくらみ/メグロコ/ワルビル/ワルビアル/ロケット団のポリゴン等）
- [x] `手札干渉>山札戻し` に「おたがい」型パターン追加: ゴヨウ/アンフェアスタンプ/ロケット団のアポロ/クラウン
- [x] `手札干渉>山札戻し` に「山札の下にもどす」型パターン追加: Nのチョロネコ/ビビヨン/ぼうがいレター/エネはたき/スペシャルレッドカード
- [x] 59件→78件（+19）合計 `card_tags.json` リビルド: 手札干渉>トラッシュ:43件/手札干渉>山札戻し:78件
- [x] `group_categories_proposal.md` §3件数・例カード・サマリー表更新

### Phase 35-6: カード種別参照>ポケモンのどうぐ 廃止 ✅ (2026-05-17)
- [x] `カード種別参照>ポケモンのどうぐ`（144件）廃止: `tag_cards.mjs` §B-28ブロック削除、GROUP_ORDER / SECTION_MAP から削除
- [x] `data/card_tags.json` 144件から当該タグを除去
...
- [x] **7-1 ID収集**: `node scripts/ptcg_crawler/harvest_ids.js` を実行し、`data/card_ids.json` を最新化する。実行前後の件数差分をログに出力する。
> [!success] 2026-04-18 — **7-1 ID収集 完了**

- [x] **7-2 詳細スクレイプ**: `node scripts/ptcg_crawler/scrape_details.js` を実行し、新カードの詳細を `data/card_details.json` に追記する（差分処理済みのため既存カードはスキップされる）。完了後に新規追加件数を報告する。
> [!success] 2026-04-18 — 完了しました。

- [x] **7-3 正規化**: `node scripts/ptcg_crawler/normalize_cards.js` を実行し、`data/master_cards.json` と `data/card_variants.json` を更新する。更新前後の master_cards・card_variants の件数差分を報告する。
> [!success] 2026-04-18 — 正規化を実行しました。結果は以下の通りです。

- [x] **7-4 タグ付け**: `node scripts/tag_cards.mjs` を実行し、新カードへのタグ付けを行う。その後 `data/card_tags.json` の「タグ付き件数 / 全体件数」を報告する。もし新しいカード種別や能力タイプが増えていてタグルールでカバーできていない場合、`scripts/tag_cards.mjs` にルールを追加してから再実行する。
> [!success] 2026-04-18 — タグ付け完了。

- [x] **7-5 QAインデックス再ビルド**: `node scripts/build_qa_index.mjs` を実行し、`data/qa_index.json` を最新化する。relatedQA ありカード数の変化を報告する。
> [!success] 2026-04-18 — `qa_index.json` を再ビルドしました。

- [x] **6o-1 §2 相手入れ替え (L81-101)**: C-04/C-05/バウンスのレビュー。`atk`のみ検査か確認し、特性版があれば `all` に変更検討。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-18 — ## §2 相手入れ替え レビュー結果

- [x] **6o-2 §3 手札干渉 (L103-144)**: トラッシュ/山札戻し (L103-129) と手札参照 (L130-144) のレビュー。乗算形式・パターン網羅性に注目。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-18 — レビュー完了。乗算形式・網羅性の観点で確認結果。

- [x] **6o-3 §4-1 ベンチ狙撃>ダメージ型 (L147-151)**: `BENCH_BRACKET` 正規表現の文字種（全角・半角括弧）の網羅性を確認。`〔〕` `［］` 以外の表現がないか調査。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-18 — 調査完了。報告します。

- [x] **6o-4 §4-3 ベンチ狙撃>移動型 (L173-177)**: 「のせ替える」以外の表現（「移す」「動かす」等）がないか調査。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-18 — ## §4-3 ベンチ狙撃>移動型 調査結果

- [x] **6o-5 §5 エネ加速 (L180-216)**: §5-A 山札→手札 (L180-183), §5-C 手札→ポケモン (L193-205), §5-D トラッシュ→ポケモン (L206-216) を一括レビュー。`{0,15}` → `{0,20}` 等の調整余地、隣接パターン漏れに注目。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-18 — ## §5 エネ加速 一括レビュー結果

- [x] **6o-6 §B-1 ドロー>サーチ系 (L228-267)**: ポケモンサーチ (L228-241), トレーナーズサーチ (L242-253), トラッシュ回収 (L254-267) を一括レビュー。「手札に加える」以外の表現（「使える」「持ってくる」等）とカード種別漏れを確認。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-18 — ## §B-1 ドロー>サーチ系 レビュー結果

- [x] **6o-7 §B-2 特殊状態付与 5種 (L270-279)**: 「どく/やけど/こんらん/ねむり/マヒ」以外の記述形式（「〜にする」のバリエーション等）がないか調査。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-18 — ## 調査結果: §B-2 特殊状態付与 パターン漏れ

- [x] **6o-8 §B-4 ダメカン操作 (L298-303)**: 「のせ替える」以外のダメカン操作表現がないか調査。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — ## §B-4 ダメカン操作 調査結果

- [x] **6o-9 §B-5 エネ移動>つけ替え (L305-310)**: 隣接制約（`{1,N}` → `{0,N}` 緩和の余地）をレビュー。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — ## §B-5 エネ移動>つけ替え レビュー結果

- [x] **6o-10 §B-6 エネ除去 (L312-323)**: 検査対象（`atk` / `abl` / `all` のどれを使っているか）の妥当性を確認。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — ## §B-6 エネ除去 スコープ検証結果

- [x] **6o-11 §B-7 ワザロック/にげられない (L326-341)**: 「にげられない」が全角表現で書かれるバリエーションがないか調査。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — 調査完了。結論: パッチ不要。

- [x] **6o-12 §B-8 グッズサポートロック (L343-358)**: 文字種「」有無は対応済み、追加パターン（「使えない」のバリエーション等）がないか調査。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — ## §B-8 グッズサポートロック 調査結果

- [x] **6o-13 §B-9 軽減 (L360-396)**: ルールテキスト vs 特性テキストの検査対象を確認。`rul` / `abl` / `all` の使い分けの妥当性を評価。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — ## Phase 6o-13 §B-9 軽減 検査対象の評価

- [x] **6o-14 §B-10 カウンター効果 (L443-446)**: 「ダメージを受けたとき」以外の発動条件表現がないか調査。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — ---

- [x] **6o-15 §B-11 自傷 (L448-461)**: 「このポケモンにも〇ダメージ」以外の自傷パターン（「自分にも〜」等）がないか調査。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — **調査・修正結果:**

- [x] **6o-16 §B-12 ベンチ展開 (L463-484)**: 「ベンチに出す」以外の表現（「ベンチに置く」「ベンチに加える」等）がないか調査。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — 分析完了。結果を整理します：

- [x] **6o-17 §B-13 進化加速 (L486-505)**: 隣接制約（`{1,N}` → `{0,N}` 緩和）の余地をレビュー。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — 結果が揃いました。レビューをまとめます。

- [x] **6o-18 §B-14/15/16 山札破壊・スタジアム・サイド操作 (L507-526)**: 3カテゴリ一括でパターン漏れ確認。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — 完了。結果まとめ：

- [x] **6o-19 §B-17 条件ダメージ (L528-587)**: 最も複雑なルール群。場参照/相手参照/コイン可変の網羅性を重点的にレビュー。サンプルカードを多めに抽出して確認。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — 十分なデータが揃いました。レビュー結果をまとめます。

- [x] **6o-20 §B-18 特殊連動 (L589-625)**: 新キャラ・新セット追加でルール漏れがないか確認。最新のカードプール（Phase 7-1〜7-4 で追加分含む）を中心に調査。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — tagging_stats.md はすでに最新（215/14/24/28/147）でした。更新不要です。

- [x] **6o-21 §B-22 特性無効 (L666-688)**: 境界ケース（特定条件下のみ無効化等）の精度を確認。誤検出・漏れ両方をレビュー。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — 分析完了。レポートをまとめます。

- [x] **6o-22 §B-26 ベンチ保護 (L709-734)**: `abl` と `rul` 両方をチェックしているか確認。漏れていれば `all` への変更を検討。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — 正常に動作しています。変更の要約：

- [x] **6o-23 §B-24 ダメージ操作>抵抗無視 (L737-745)**: 検査対象が `atk` のみで適切か確認。特性版があるか調査。手順は Phase 6o 共通手順に従う。
> [!success] 2026-04-19 — 完了しました。

- [x] **6o-24 全体検証**: 6o-1〜6o-23 完了後、`node scripts/tag_cards.mjs` を最終実行し、`docs/tagging_stats.md` を生成。修正前後のタグ付き件数の総合差分を `docs/tag_review_phase6o_summary.md` にまとめて報告。
> [!success] 2026-04-19 — Phase 6o 全体検証完了です。結果は以下の通りです。

- [x] **10-1** AND/OR タグトグル実装（CardSearch.tsx） model:sonnet
> [!success] 2026-04-23 — 実装完了です。変更内容をまとめます：

- [x] **10-2** タグ「にげロック」分離（tag_cards.mjs + frontend） model:sonnet
> [!success] 2026-04-24 — 完了です。変更サマリー：

- [x] **10-3** エネルギーコスト検索実装（build_cost_index.mjs + CardSearch.tsx） model:sonnet
> [!success] 2026-04-24 — 実装完了です。変更内容のまとめ:

- [x] **10-4** Phase 10 レビュー・修正・commit/push model:sonnet
> [!success] 2026-04-24 — commit & push 完了です。

- [x] **11-1** cost_index 構造改修 + エネフィルター AND修正（build_cost_index.mjs + CardSearch.tsx） model:sonnet
...
- [x] **11-2** 詳細フィルターパネル整理（名称変更・テクニック廃止・全解除追加） model:sonnet
> [!success] 2026-04-26 — 変更内容が正しく適用されています。完了です。
> [!failure] 2026-04-25 — Failed to authenticate. API Error: 401 {"type":"error","error":{"type":"authentication_error","message":"Invalid auth...
> [!failure] 2026-04-25 — Failed to authenticate. API Error: 401 {"type":"error","error":{"type":"authentication_error","message":"Invalid auth...

- [x] **11-3** タイプ・弱点・抵抗力の日本語表示（CardSearch + CardModal） model:sonnet
> [!success] 2026-04-26 — 型エラーなし。変更内容をまとめて確認します。
> [!failure] 2026-04-25 — Failed to authenticate. API Error: 401 {"type":"error","error":{"type":"authentication_error","message":"Invalid auth...
> [!failure] 2026-04-25 — Failed to authenticate. API Error: 401 {"type":"error","error":{"type":"authentication_error","message":"Invalid auth...

- [x] **11-4** 画像読み込み失敗時フォールバック（CardSearch + CardModal） model:sonnet
> [!success] 2026-04-26 — 型エラーなし。変更内容まとめ：

- [x] **11-5** Phase 11 レビュー・修正・commit/push model:sonnet
> [!success] 2026-04-26 — コミット・プッシュ完了です。

- [x] **12-1** タグ50音順ソート（CardSearch.tsx） model:sonnet
> [!success] 2026-04-25 — 親タグ（line 160）とサブタグ（line 616）の両方を `localeCompare('ja')` に変更しました。Unicode コードポイント順から日本語の50音順ソートになります。

- [x] **12-2** 展開中タグの視覚強調（CardSearch.tsx） model:sonnet
> [!success] 2026-04-25 — 変更内容：

- [x] **12-3** タグ件数バッジ表示（CardSearch.tsx） model:sonnet
> [!success] 2026-04-25 — 完了しました。変更内容：

- [x] **12-4** Phase 12 レビュー・commit/push model:sonnet
> [!success] 2026-04-25 — 完了です。変更内容のサマリー：

- [x] **13-1** マスターカードへのタグ集約（CardSearch.tsx） model:sonnet
> [!success] 2026-04-27 — 完了しました。変更内容のまとめです：

- [x] **13-2** タグ件数バッジをバリアント数ベースに修正（CardSearch.tsx） model:sonnet
> [!success] 2026-04-27 — 完了しました。修正内容：

- [x] **13-3** Phase 13 レビュー・commit/push model:sonnet
> [!success] 2026-04-27 — 完了です。

- [x] **14-1** CardModal QA表示実装 model:sonnet
> [!success] 2026-04-28 — 完了です。変更点のまとめ：

- [x] **14-2** Phase 14 レビュー・commit/push model:sonnet
> [!success] 2026-04-28 — Phase 14 レビュー完了。変更のまとめ：

- [x] **15-1** 上位バリアント表示（CardModal） model:sonnet
> [!success] 2026-04-29 — 完了しました。変更内容：

- [x] **15-2** QAセクション改善（CardModal） model:sonnet
> [!success] 2026-04-29 — 完了です。変更内容のまとめ：

- [x] **15-3** バリアント全件モーダル（CardModal） model:sonnet
> [!success] 2026-04-29 — 完了です。

- [x] **15-4** Phase 15 レビュー・commit/push model:sonnet
> [!success] 2026-04-29 — 完了。コミット・プッシュしました。

- [x] **6n-1 調査・設計**: `data/card_details.json` から「山札を引く」系テキストを持つカードをサンプル抽出し、現行 `ドロー>枚数固定` `ドロー>手札補充` `ドロー>コイン可変` の3タグで完全カバーできているか確認する。カバーできていないパターンを列挙し、追加・修正が必要なタグルール案を `docs/` に出力する（新規ファイル名は任意）。
> [!success] 2026-04-30 — 調査結果をまとめました。

- [x] **6n-2 実装**: 6n-1 の調査結果ファイルを読み、提案されたタグルール修正を `scripts/tag_cards.mjs` に適用する。適用後 `node scripts/tag_cards.mjs` を実行し、before/after の `ドロー` 系タグ付き件数差分（`git diff data/card_tags.json | grep '^[+-]' | grep -c '"draw'` 等）を確認して誤検出がないことを検証する。問題なければ `git add data/card_tags.json scripts/tag_cards.mjs && git commit && git push` する。
> [!success] 2026-04-30 — 完了しました。変更内容のまとめ：

- [x] **6m-1** 「たねポケモン」「1進化」「2進化」「ex」「V/VMAX/VSTAR」「GX」のカードカテゴリタグ追加（tag_cards.mjs + card_tags.json + git push）。`scripts/tag_cards.mjs` に新規タグルールセクション「カードカテゴリ」を追加し、以下のタグを定義すること: `カードカテゴリ>たねポケモン`（`supertype === "Pokémon"` かつ `subtypes` に `"Basic"` を含む）、`カードカテゴリ>1進化`（`subtypes` に `"Stage 1"` を含む）、`カードカテゴリ>2進化`（`subtypes` に `"Stage 2"` を含む）、`カードカテゴリ>ex`（`name` が `/\sex$|\sex\s/i` にマッチ）、`カードカテゴリ>V/VMAX/VSTAR`（`subtypes` に `"V"`, `"VMAX"`, `"VSTAR"` のいずれかを含む）、`カードカテゴリ>GX`（`subtypes` に `"GX"` を含む）。実行後に各タグの件数を報告し、`git add data/card_tags.json scripts/tag_cards.mjs && git commit -m "Add card category tags" && git push`。 model:sonnet
> [!success] 2026-04-30 — 完了しました。変更内容の詳細：

- [x] **15.5-A** トレーナーカード重複統合（normalize_cards.js の createMasterKey 修正 + pipeline 再実行）。`scripts/ptcg_crawler/normalize_cards.js` の `createMasterKey(card)` を修正: `hp === null` かつ `attacks.length === 0` かつ `abilities.length === 0` の場合は `rules` を key に含めず `${card.name}_none_none_none_none_none` を返す。修正後 `node scripts/ptcg_crawler/normalize_cards.js` → `node scripts/tag_cards.mjs` → `node scripts/build_qa_index.mjs` を順に実行。完了後に master_cards.json の件数変化、`きずぐすり`・`ふしぎなアメ` が各1件になっているかを確認し、`git add` → `git commit` → `git push` して報告する。 model:sonnet
> [!success] 2026-05-01 — 完了です。

- [x] **15.5-B** CardModal バリアントサムネイル `sm:min-h-0` 追加（CardModal.tsx）。`frontend/app/components/CardModal.tsx` のバリアントサムネイル container div（`className="px-2 pb-2 sm:p-4 bg-gray-950 sm:flex-grow sm:overflow-y-auto custom-scrollbar border-t border-gray-800/40 sm:border-t-0 overflow-hidden"`）に `sm:min-h-0` を追加する。`npm run build` → `git commit` → `git push`。完了後に変更箇所と push 結果を報告。 model:sonnet
> [!success] 2026-05-01 — 完了。

- [x] **15.5-C** variantUtils.ts `genRank` を修正して SWSH era の全set_code（S系+MC/MP系）を rank 5 に。`frontend/lib/variantUtils.ts` の `genRank()` を修正: `/^SV/.test` → rank 6、`/^SM/.test` → rank 4、`/^S/.test` → rank 5（SV/SM 以外の S系全部）、`/^XY/.test` → rank 3、`/^BW/.test` → rank 2、`/^MC|^MP/.test` → rank 5、その他 → rank 1 の順で判定する。`npm run build` → `git commit` → `git push`。変更後 `きずぐすり` のモーダルで `スタートデッキ100 バトルコレクション` バリアントのサムネイルが正しく表示されることを確認して報告。 model:sonnet
...
- [x] **後-B** グッズ/サポート/スタジアム参照タグの全件漏れチェック・補完
...
- [x] **20-4** Phase 20 レビュー・修正・commit/push
> [!success] 2026-05-07 — 完了。Phase 20の3タスクをまとめてコミット・プッシュしました。

#### Phase 21-5: Phase 21 レビュー・修正・commit/push

- [x] **21-5** Phase 21 総合レビュー。`data/deck_index.json` でドラパルトex・カビゴンex 等メジャーカードが複数アーキタイプを持つことを確認。CardModal でアーキタイプが正しく表示されること・limitlesstcg リンクが正しいことを確認。採用デッキがないカード（例: 旧世代限定カード）でセクションが非表示になることを確認。`npm run build` → 問題あれば修正 → `git push`。
...
- [x] **22-1** `frontend/lib/variantUtils.ts` の `pickDefaultVariant` 改修（SAR/AR優先・プロモ除外廃止・ソート簡略化） model:sonnet
...
- [x] **23-1** official_class_index を official_id ベースに変更（builder + frontend）
...
- [x] **23-2** `officialIds` メカニズム追加 + テラスタル/古代/未来を確定 official_id リストに移行
...
- [x] **23-3** prefix cardKindFilter 追加 + rulesContain 拾い漏れ修正 + ロケット団エネルギー追加
> [!success] 2026-05-09 — 3件とも正しく修正されています。

#### Phase 23-4: Phase 23 レビュー・commit/push

- [x] **23-4** Phase 23 総合レビュー。`npm run build` エラーなし確認 → `git commit` → `git push`
...
- [x] **24-1** ベースライン取得（全タグのカード数・Q&A数を docs/tag_migration_baseline.md に記録）+ マギアナ/エネルギーリサイクル/レガシーエネルギー/ビワ/ピュールのパターンマッチ問題を tag_cards.mjs で原因特定・記録 + 24-7 で使う検証計画（スポットチェックリスト・件数許容範囲・Q&A閾値・廃止タグリスト）を同ファイルに設計・記録。コード変更はしない
...
- [x] **24-2** グッズサポートロック→ロック・サイド操作→サイド関連・スタジアム>破壊→トラッシュ・サーチ>自由選択→好きなカード・にげるコスト無償+にげロック→逃げる操作（統合）・ベンチ展開>リバイバル廃止・ACE SPEC廃止。tag_cards.mjs と tag_qa_entries.mjs を同期更新し VALID_TAGS 更新後リビルド
...
- [x] **24-3** エネ移動解体（→エネ加速>ポケモン / エネ除去>つけかえ）・エネ加速サブタグ改名（→ポケモン削除）・マギアナ除外・エネ加速>山札→手札→サーチ>エネルギー移動・エネルギーリサイクル→トラッシュ回収・メガフシギバナex追加。tag_cards.mjs と tag_qa_entries.mjs 同期更新・VALID_TAGS更新・リビルド
...
- [x] **24-4** サーチ>スタジアム新設（グッズから分離）・サーチ>ポケモンのどうぐ新設（ポケモンから分離）・ヨルノズク/ロケット団のラムダ/カキツバタ等トレーナーズ全般サーチカードへの全サブタグ付与。tag_cards.mjs と tag_qa_entries.mjs 同期更新・リビルド
...
- [x] **24-5** トラッシュ回収サブタグを「場/手札/山札」に再分類・レガシーエネルギー除外・どうぐトラッシュ廃止→ポケモンのどうぐ>トラッシュ新設。tag_cards.mjs と tag_qa_entries.mjs 同期更新・VALID_TAGS更新・リビルド
...
- [x] **24-6** ビワを正しいタグに修正・ピュールにドロー>手札トラッシュ後追加（パターン修正）。tag_cards.mjs と tag_qa_entries.mjs 同期更新・リビルド
...
- [x] **24-7** ベースラインとの差分比較・スポットチェック・Q&Aカバレッジ確認を実施し docs/tag_migration_verification.md に検証レポート出力
> [!success] 2026-05-09 — `docs/tag_migration_verification.md` を作成しました。

#### Phase 24-8: Phase 24 レビュー・commit/push

- [x] **24-8** Phase 24 総合レビュー。`npm run build` エラーなし確認 → `git add` → `git commit` → `git push`
> [!success] 2026-05-09 — 完了。
...
- [x] **25-1** ベースライン取得（全対象タグのカード数・Q&A数）+ ダメカン操作カード全件の分類案 + ベンチ狙撃>移動型リスト + 条件ダメージパターン漏れ調査。docs/tag_phase25_baseline.md に出力。コード変更なし
...
- [x] **25-2** ダメカン操作>のせる/移動/反射 サブタグ新設 + ベンチ狙撃>移動型廃止。tag_cards.mjs と tag_qa_entries.mjs 同期更新・VALID_TAGS更新・リビルド
...
- [x] **25-3** ダメージ操作>与ダメ強化→与ダメージ増加・被ダメ増加→被ダメージ増加 改名 + 条件ダメージパターン補完（25-1調査結果ベース）。tag_cards.mjs と tag_qa_entries.mjs 同期更新・リビルド
...
- [x] **25-4** ベースラインとの差分比較・スポットチェック・廃止タグ確認を実施し docs/tag_phase25_verification.md に出力
> [!success] 2026-05-11 — `docs/tag_phase25_verification.md` を作成しました。

#### Phase 25-5: レビュー・commit/push

- [x] **25-5** Phase 25 総合レビュー。`npm run build` エラーなし確認 → `git add` → `git commit` → `git push`
...
- [x] **26-1** variantUtils.ts 改修（発売日ベース新しさ優先・同世代ノーマル優先）。npm run build 確認 → git commit → git push
...
- [x] **27-1** リセット・全解除ボタン出現によるUIずれ修正（事前に領域確保）。npm run build 確認
> [!success] 2026-05-13
...
- [x] **27-2** セレビィのタグ確認・修正（サーチ>ポケモン追加検討）。リビルド → git push
> [!success] 2026-05-13
...
- [x] **28-3** 最大HP強化タグ新設（§B-29）。tag_cards.mjs + tag_qa_entries.mjs 同期・リビルド → git push
...
- [x] **28-3** 最大HP強化タグ新設（最大HP+N パターン追加）。tag_cards.mjs + tag_qa_entries.mjs 同期・VALID_TAGS更新・リビルド → git push
> [!success] 2026-05-14
...
- [x] **29-1** テキスト検索復活（effectQuery アンコメント + 特性名/ワザ名/テキスト検索ロジック実装）。npm run build 確認 → git commit → git push
> [!success] 2026-05-14
...
- [x] **30-1** 最終進化フィルター実装（isFinalEvolution ロジック + finalEvoOnly トグル UI）。npm run build 確認 → git commit → git push
> [!success] 2026-05-14
...
- [x] **30-2** CardModal 採用デッキセクションのラベルを日本語化。npm run build 確認
> [!success] 2026-05-14
...
- [x] **27-3** ワザコピータグ：ヤドキング調査・追加、他漏れ確認。リビルド → git push
> [!success] 2026-05-16
...
- [x] **27-4** Phase 27 総合レビュー。`npm run build` エラーなし確認 → `git push`
...
- [x] **27-4** Phase 27 総合レビュー。`npm run build` エラーなし確認 → `git push`（未push分があれば）
> [!success] 2026-05-16
...
- [x] **27-5** グロウ草エネルギー誤タグ削除・軽減>exVGXから dead tag削除・無効>条件付きその他→条件付きリネーム。リビルド → git push
> [!success] 2026-05-16
...
- [x] **28-1** ワザロック廃止（>相手ロック→ロック付与後にワザロック全削除、>自己ロック削除）。同期更新・リビルド → git push
> [!success] 2026-05-16
...
- [x] **28-2** 手札干渉サブタグ廃止（手札トラッシュ・手札参照）。同期更新・リビルド → git push
> [!success] 2026-05-16
...
- [x] **28-4** 進化加速整理（>山札から廃止 + イーブイ確認）。同期更新・リビルド → git push
> [!success] 2026-05-16
...
- [x] **28-5** 特性無効→ロック>特性 統合（新サブタグ追加・特性無効廃止）。同期更新・リビルド → git push
> [!success] 2026-05-16
...
- [x] **28-6** 回復>複数回復廃止（再分類後に削除）。同期更新・リビルド → git push
> [!success] 2026-05-17
...
- [x] **31-3** ベンチ展開サブタグ追加（>山札・>トラッシュ）。同期更新・リビルド → git push
> [!success] 2026-05-16
...
- [x] **31-1** トラッシュ回収>場 削除（69件をベンチ展開>トラッシュ/エネ加速>トラッシュに振替後、ルール削除）。同期更新・リビルド → git push
> [!success] 2026-05-16
...
- [x] **31-2** ハルクジラex/ムゲンダイナ にスタジアム参照タグ付与（スピンロトム同等扱い）。同期更新・リビルド → git push
> [!success] 2026-05-16
...
- [x] **31-4** 回復>特殊状態 サブタグ新設。同期更新・リビルド → git push
> [!success] 2026-05-16
...
- [x] **31-5** にげる操作→にげるコスト リネーム + サブタグ 軽減/増加 整理。同期更新・リビルド → git push
> [!success] 2026-05-16
...
- [x] **31-6** Phase 31 レビュー・廃止タグ確認・push
> [!success] 2026-05-16 — Phase 31 レビュー完了。廃止タグが card_tags.json / qa_entry_tags.json に残存しないことを確認。npm run build 正常
...
- [x] **32-1** ポケモンいれかえ サブタグ一新 + 相手入れ替え統合（自分/味方/相手:C-04型/C-05型/バウンス）。同期更新・リビルド → git push
> [!success] 2026-05-16
...
- [x] **32-2** ロックタグ大規模整理（スタジアム/どうぐ/にげられない/にげられない（自分）/ワザ サブタグ追加・元タグから移行）。同期更新・リビルド → git push
> [!success] 2026-05-16 — ロックタグ大規模整理完了。スタジアム/どうぐ/にげられない/ワザ サブタグ追加・移行。リビルド・git push 完了
...
- [x] **32-3** 特殊状態関連タグ新設（特殊水エネルギー等の特殊状態参照カード分類）。同期更新・リビルド → git push
> [!success] 2026-05-16 — 特殊状態関連タグ新設完了。特殊水エネルギー等の特殊状態参照カード分類。リビルド・git push 完了
...
- [x] **32-4** カード種別参照タグ新設（ポケモンex参照等のサブタグ統合）。同期更新・リビルド → git push
> [!success] 2026-05-16 — カード種別参照タグ新設完了。ポケモンex参照等12サブタグ統合。リビルド・git push 完了
...
- [x] **28-6** 回復>複数回復廃止（再分類後に削除）。同期更新・リビルド → git push
> [!success] 2026-05-16 — 回復>複数回復廃止完了。再分類後に削除。リビルド・git push 完了
...
- [x] **28-7** Phase 28 レビュー・廃止タグ全カード/Q&A 確認・push
> [!success] 2026-05-16 — Phase 28 レビュー完了。廃止タグが card_tags.json / qa_entry_tags.json に残存しないことを確認
...
- [x] **31-7** トラッシュ回収>山札 漏れ補完（せいなるはい等の調査・追加）。同期更新・リビルド → git push
> [!success] 2026-05-16 — トラッシュ回収>山札 漏れ補完完了。せいなるはい×4 追加。リビルド・git push 完了
...
- [x] **31-8** 進化加速 グッズ・スタジアム補完（活力の森追加・パターン拡張）。同期更新・リビルド → git push
> [!success] 2026-05-17
#### Phase 31-8: 進化加速 グッズ・スタジアム補完
...
- [x] **31-8** 進化加速 グッズ・スタジアム補完（ふしぎなアメ等の調査・追加）。同期更新・リビルド → git push
> [!success] 2026-05-16 — 進化加速 グッズ・スタジアム補完完了。ふしぎなアメ等追加。リビルド・git push 完了
...
- [x] **31-9** サーチ>スタジアム アクロマの執念追加（他漏れ確認含む）。同期更新・リビルド → git push
> [!success] 2026-05-16 — サーチ>スタジアム 漏れ補完完了。アクロマの執念×3・シークレットボックス×1 追加。リビルド・git push 完了
...
- [x] **32-5** ポケモンのどうぐ整理（>参照 サブタグ追加、参照系カードを移行）。同期更新・リビルド → git push
> [!success] 2026-05-16 — ポケモンのどうぐ>参照 サブタグ新設完了。14件新設 / カード種別参照>ポケモンのどうぐ 158件→144件。git push 完了
...
- [x] **32-6** Phase 32 レビュー・廃止タグ全カード/Q&A 確認・push
> [!success] 2026-05-16 — Phase 32 レビュー完了。廃止タグが card_tags.json / qa_entry_tags.json に残存しないことを確認
...
- [x] **35-1** 独自タグ「フィールド干渉」を新設（サブタグ: エネルギーつけかえ/エネルギートラッシュ/スタジアムトラッシュ/どうぐトラッシュ）し、エネ除去タグを削除・各サブタグを移動先へ振り分ける。カード種別参照>どうぐトラッシュもフィールド干渉>どうぐトラッシュに移動する
> [!success] 2026-05-17 — pickDefaultVariant→pickDisplayVariant リネーム・ロジック置換完了。eraRank/rarityRank/NORMAL_RARITY/releaseDB import と sort ロジック削除。CardSearch.tsx・CardModal.tsx 参照更新
...
- [x] **35-2** 独自タグ「スタジアム」を削除し、スタジアム>封印→ロック>スタジアム（新設）、スタジアム>参照→カード種別参照、スタジアム>トラッシュ→フィールド干渉>スタジアムトラッシュに移動する
> [!success] 2026-05-17 — pickDefaultVariant→pickDisplayVariant リネーム・ロジック置換完了。eraRank/rarityRank/NORMAL_RARITY/releaseDB import と sort ロジック削除。CardSearch.tsx・CardModal.tsx 参照更新
...
- [x] **35-3** ワザロック(相手)をロックに統合し、ロックのサブタグ名から「ロック」の文字を除去し（グッズロック→グッズ等）、ロック>トレーナー効果無効を無効>グッズ/無効>サポートへ移動する
> [!success] 2026-05-17 — ワザロック(相手)廃止・ロックサブタグ名整理・トレーナー効果無効を無効>グッズ/サポートへ移動完了
...
- [x] **35-4** 独自タグ「ポケモンチェック」を削除してダメカン操作>のせるに吸収し、「にげられない」をロック>にげられないに、「にげられない（自分）」を回復>にげられない（自分）に移動する
> [!success] 2026-05-17 — 独自タグ「ポケモンチェック」廃止・ダメカン操作>のせるに吸収。にげられない→ロック>にげられない移動完了
...
- [x] **35-5** ポケモンいれかえ>自分の分類ミスを>味方に修正し、サーファーのマッチ漏れを追加する
> [!success] 2026-05-17 — §1-A を「このポケモン」ワザ限定に絞り込み / §1-B に abl パターン追加・名前リスト拡張 / 自分203→31件、味方7→205件
#### Phase 35-5: ポケモンいれかえ 分類ミス修正 + サーファー追加

以下のカードが現在 ポケモンいれかえ>自分 に入っているが、正しくは ポケモンいれかえ>味方 である:
- ポケモンいれかえ（カード名）、スクランブルスイッチ、なみのりビーチ、ダイケンキ、メガカイリューex

マッチ漏れ: サーファーが ポケモンいれかえ>味方 に入っていない。スタジアムや特殊エネルギーも含め全カード対象で漏れを確認すること。

- [x] **35-5** ポケモンいれかえ>自分 の分類ミス（ポケモンいれかえ/スクランブルスイッチ/なみのりビーチ/ダイケンキ/メガカイリューex）を >味方 に修正し、サーファーのマッチ漏れを追加する
> [!success] 2026-05-17 — ポケモンいれかえ>自分 分類ミス修正・サーファーのマッチ漏れ補完完了

#### Phase 35-6: カード種別参照>ポケモンのどうぐ 廃止

公式タグおよび他の独自タグに役割があるため廃止する。

- [x] **35-6** カード種別参照>ポケモンのどうぐ を廃止し、タグルールと付与済みタグデータから削除する
> [!success] 2026-05-17 — カード種別参照>ポケモンのどうぐ 廃止完了。144件からタグを除去


#### Phase 35-8: 独自タグ「自傷」廃止

自傷>ダメージ → 条件ダメージ>自傷（§B-17-8新設）、自傷>ダメカン → ダメカン操作>のせる（§B-4-3統合）

- [x] **35-8** 独自タグ「自傷」を廃止し、自傷>ダメージを条件ダメージ>自傷に、自傷>ダメカンをダメカン操作>のせるに移動する
> [!success] 2026-05-17 — 自傷>ダメージ131件→条件ダメージ>自傷 / 自傷>ダメカン5件→ダメカン操作>のせる（ウネルミナモ対応パターン追加）/ §B-11廃止
#### Phase 35-8: 自傷タグ削除・分散

自傷タグ自体を廃止し、各サブタグを適切な既存タグに移動する。

- 自傷>ダメージ（「このポケモンにも〇ダメージ」「このポケモンに〇ダメージ」）→ 条件ダメージ
- 自傷>ダメカン（「このポケモンにダメカンを〇個のせ」等）→ ダメカン操作>のせる

自傷タグのパターンルール自体は tag_cards.mjs から削除し、同等のパターンが条件ダメージ・ダメカン操作側でカバーされているか確認・補完すること。リビルド必須。

- [x] **35-8** 独自タグ「自傷」を廃止し、自傷>ダメージを条件ダメージに、自傷>ダメカンをダメカン操作>のせるに移動する
> [!success] 2026-05-17 — 独自タグ「自傷」廃止完了。自傷>ダメージ→条件ダメージ / 自傷>ダメカン→ダメカン操作>のせる 移動

#### Phase 35-9: 手札干渉>山札戻し・トラッシュ パターン補完

以下のカードが現状どのサブタグにもマッチしていない。パターンを追加する。

手札干渉>山札戻し に追加:
- 「おたがいのプレイヤーは、それぞれ手札をすべて山札にもどして切る」型（アンフェアスタンプ、クラウン等）→ `/おたがい.{0,30}手札.{0,30}山札にもどして切る/` を追加
- 「相手は手札をすべてウラにして切り、山札の下にもどす」型（スペシャルレッドカード）→ `/相手.{0,5}手札.{0,30}山札の下にもどす/` を追加

手札干渉>トラッシュ に追加:
- 「相手は相手自身の手札を〇枚になるようにトラッシュする」型（クセロシキのたくらみ）→ `/相手は相手自身の手札を.{1,30}トラッシュ/` を追加

これらのカード以外にも同型のカードがあれば合わせて対応すること。リビルド必須。

- [x] **35-9** 手札干渉>山札戻しに「おたがい」型・「山札の下にもどす」型パターンを追加し、手札干渉>トラッシュに「相手は相手自身の手札を〜トラッシュ」型パターンを追加する
> [!success] 2026-05-17 — 手札干渉パターン拡充完了。トラッシュ:30→43件 / 山札戻し:59→78件

#### Phase 35-7: カウンター効果リネーム + 条件絞り込み

カウンター効果 → カウンター効果（ダメカン以外）にリネームし、ダメカン操作>反射と重複するカード（ダメカンをのせる系）を対象から除外するよう条件を絞る。ダメカン以外のカウンター（ダメージで反撃、状態異常付与、ドローなど）のみを対象にする。`HANSHA_PAT` にマッチするケースを除外した条件にすること。

- [x] **35-7** 独自タグ「カウンター効果」を「カウンター効果（ダメカン以外）」にリネームし、ダメカン操作>反射と重複しないよう条件を絞り込む
> [!success] 2026-05-17 — カウンター効果→カウンター効果（ダメカン以外）リネーム完了。ダメカン反射型33件を分離し16件に絞り込み

#### Phase 34-1: テキスト検索でスタジアム・特殊エネルギーがヒットしない修正

✅ **完了（Gardener直接実行）**

`frontend/app/components/CardSearch.tsx` の effectText 構築に `...(card.rules || [])` を追加。

- [x] **34-1** `frontend/app/components/CardSearch.tsx` の effectText 構築に `...(card.rules || [])` を追加し、スタジアム・特殊エネルギーのテキスト検索ヒットを修正する
> [!success] 2026-05-17 — CardSearch.tsx effectText に card.rules を追加しスタジアム・特殊エネルギーのテキスト検索ヒットを修正

#### Phase 34-3: tag_cards.mjs の atk/abl に rul を追加（全タグでrulesテキストを検索対象に）

✅ **完了（Gardener直接実行）**

`scripts/tag_cards.mjs` の `assignTags` 関数内で `atk`/`abl` に `rul` を付加。card_tags.json リビルド済み。

- [x] **34-3** `scripts/tag_cards.mjs` の `assignTags` 関数内で `atk` と `abl` に `rul` を付加し、全タグパターンがrulesテキストも検索するように修正する
> [!success] 2026-05-17 — tag_cards.mjs assignTags の atk/abl に rul を付加し全タグがrulesテキストを検索対象にした

#### Phase 34-2: 古いカードの表示問題を修正

✅ **完了（Gardener直接実行）**

原因: `pickDefaultVariant`（`frontend/lib/variantUtils.ts`）がアルファベット降順でソートしていたため、'XYP' > 'SVI' となり旧時代バリアントが表示されていた。

修正: `eraRank()` 関数を追加（SV*=0, MP*=1, M*=2, その他=99）し、エラ優先でソート。DBには H/I/J レギュのカードのみ収録済みで、SVフィルタ（page.tsx）は不要だったため削除。

- [x] **34-2** 古い印刷物（XYP等）が表示される問題を修正：`pickDefaultVariant` に eraRank を追加し、SV時代バリアントを優先表示するよう修正する
> [!success] 2026-05-17 — variantUtils.ts に eraRank() を追加し SV 時代バリアントを優先表示するよう修正

#### Phase 34-4: pickDefaultVariant を実発売日ベースのソートに改善

✅ **完了（Gardener直接実行）**

公式サイトDOMからパック発売日を取得し `data/pack_release_order.json` を生成（HIJ レギュ以降の32パック）。`pickDefaultVariant` を `release_date` 文字列による降順ソートに変更。DBに日付がないコード（プロモ等）は従来の eraRank → set_code → rarityRank にフォールバック。

- [x] **34-4** 公式サイトから発売日データを取得して `data/pack_release_order.json` を作成し、`pickDefaultVariant` を実発売日ベースのソートに切り替える
> [!success] 2026-05-17 — pack_release_order.json を生成し pickDefaultVariant を実発売日ベースのソートに切り替え

#### Phase 36-1: バリアント表示ロジックを variants[0] ベースに置換

**根本原因**: `frontend/lib/variantUtils.ts` の `pickDefaultVariant` 内の `eraRank` が `SV(0) → MP(1) → M(2)` の順で SV* を最優先しているが、実際は **M*（MEGA era）の方が SV* より新しい**（MC=2025-12 vs SVHM=2024頃）。Phase 34-2/34-4 で era 順を逆向きに設計してしまったのが原因。

**設計方針**: `scripts/ptcg_crawler/normalize_cards.js:166-178` で `card_variants.json` 生成時に既に `official_id` 降順（新しい順）＋同セット内通常レア優先でソート済み。よって複雑なソートロジックは不要で、配列を順に走査して「特殊レア以外の最初の variant」を返せばよい。

**実装**:
1. `pickDefaultVariant` を `pickDisplayVariant` にリネーム（責務が「ソート→選択」から「走査→選択」に変わるため）
2. ロジック: `variants` を順に走査し、`rarity` が以下の特殊レア集合に**含まれない**最初の variant を返す。全て特殊レアなら `variants[0]` をフォールバック
3. 特殊レア集合: `{'AR', 'SR', 'SAR', 'UR', 'MA', 'MUR', 'TR', 'BWR'}`（`RR` は通常扱い—ポケモンexの標準印刷で絵柄もC/Uと同等のため）
4. 不要コード削除: `eraRank`, `rarityRank`, `NORMAL_RARITY`, `releaseDB` import、既存の sort ロジック
5. `data/pack_release_order.json` ファイル自体は**残す**（リポジトリ外データのため保持）
6. 呼び出し側更新: `CardSearch.tsx:856`（一覧グリッド）と `CardModal.tsx:97`（初期選択）の `pickDefaultVariant` 参照を `pickDisplayVariant` に変更

**期待される結果**: エネルギーつけかえが `[MC]` 表示、せいなるはい が `[M2a]`、ハイパーボール が `[MP1]` 表示になる（268件/1860件 のカードで variants[0] が AR+ のためスキップ動作）。

- [x] **36-1** `frontend/lib/variantUtils.ts` の `pickDefaultVariant` を `pickDisplayVariant` にリネームし、ロジックを「`variants` を順に走査し、`rarity` が `{AR,SR,SAR,UR,MA,MUR,TR,BWR}` に含まれない最初の variant を返す（該当なしなら `variants[0]` フォールバック、RR は通常扱い）」に置換。`eraRank`/`rarityRank`/`NORMAL_RARITY`/`releaseDB` import と既存 sort ロジックを削除（`data/pack_release_order.json` ファイル自体は残す）。`CardSearch.tsx`・`CardModal.tsx` の参照も更新
> [!success] 2026-05-17 — pickDefaultVariant→pickDisplayVariant リネーム・ロジック置換完了。eraRank/rarityRank/NORMAL_RARITY/releaseDB import と sort ロジック削除。CardSearch.tsx・CardModal.tsx 参照更新

#### Phase 36-2: 「最終進化のみ」フィルターをカード種別グループに移動

公式フィルターの「最終進化のみ」チェックボックスをカード種別グループ（ポケモン・トレーナーズ・エネルギーと並ぶグループ）内に移動する。現在の表示位置を確認してから移動すること。

- [x] **36-2** 「最終進化のみ」フィルターをカード種別グループ内に移動する #claude/queue
> [!success] 2026-05-17

#### Phase 36-3: タイプフィルターの「無職」→「ノーマル」表記変更

公式フィルターのタイプ欄に表示される「無職」を「ノーマル」に変更する。ワザやエネルギー側の「無色エネルギー」表記はそのまま変更しない。

- [x] **36-3** タイプフィルターの「無職」表記を「ノーマル」に変更する（ワザ・エネルギー側の「無色エネルギー」は変更しない） #claude/queue
> [!success] 2026-05-17

#### Phase 36-4: テキスト検索で汎用テキストをヒット除外

「グッズは、自分の番に何枚でも使える。」「スタジアムは、自分の番に1枚……」等のカード種別汎用テキストがテキスト検索にヒットしないようにする。汎用テキストの判定方法を設計して実装すること。

- [x] **36-4** テキスト検索でカード種別汎用テキスト（グッズ・スタジアム等の定型文）がヒットしないよう除外ロジックを追加する #claude/queue
> [!success] 2026-05-17

#### Phase 36-5: フィルターキャンセル時の同期バグ調査

フィルターをクリックしてキャンセルしたとき、フィルター表示と実際のフィルター状態が一致しない可能性がある。再現条件を探りつつコードを確認し、バグがあれば修正する。軽めの調査でよい。

- [x] **36-5** フィルターキャンセル時に表示と実フィルター状態がずれるバグを調査し、再現すれば修正する #claude/queue
> [!success] 2026-05-17


#### Phase 37-1: エネ加速>つけかえ サブタグ新設

自分の場のポケモン間でエネルギーを移動する効果（エネルギーつけかえ、スクランブルスイッチ、テツノイサハex等）を `エネ加速>つけかえ` として分類。相手対象のつけかえ（ゲンガーex系）は除外。

- [x] **37-1** 自分の場のエネルギーつけかえ効果を持つカードを調査し、`エネ加速>つけかえ` サブタグを新設して分類する #claude/queue
> [!success] 2026-05-17 — 96件（26ユニーク名）タグ付け完了。patterns: `/エネルギー」?を.{1,30}つけ替える/` かつ相手対象除外
#### Phase 37-1: エネ加速>つけかえ 追加

Phase 35-1 でエネ除去>つけかえ（自分の場のエネルギー移動）をエネ加速>つけかえに移動する予定だったが作業漏れ。「エネルギーつけかえ（カード名）」「メガフシギバナex」等の自分の場のエネルギーを付け替える効果を `エネ加速>つけかえ` に分類する。まず対象カードを調査してからタグ追加すること。

- [x] **37-1** 自分の場のエネルギーつけかえ効果を持つカードを調査し、`エネ加速>つけかえ` サブタグを新設して分類する #claude/queue
> [!success] 2026-05-17

#### Phase 37-2: カード種別参照>スタジアム の誤分類修正

`カード種別参照>スタジアム` に手札からエネルギーをトラッシュする系のカードが誤って含まれている。対象カードを特定し、`条件ダメージ>エネルギー消費：手札` サブタグ（新設）に移動する。検索ロジックを修正してリビルドすること。

- [x] **37-2** `カード種別参照>スタジアム` から誤分類の手札エネトラッシュ系カードを特定し、`条件ダメージ>エネルギー消費：手札`（新設）に移動する #claude/queue
> [!success] 2026-05-17

#### Phase 37-3: フィールド干渉>エネルギーつけかえ を相手の場のみに絞り込み

現在 `フィールド干渉>エネルギーつけかえ` に自分の場のエネルギー移動系（エネルギーつけかえカード・スクランブルスイッチ等）が含まれている。これらは Phase 37-1 で `エネ加速>つけかえ` に移す。`フィールド干渉>エネルギーつけかえ` には「ゲンガーex」「ハンディサーキュレーター」等、真に相手の場のエネルギーを操作するカードのみを残す。37-1 完了後に実施することを推奨するが、依存関係は強制しない。

- [x] **37-3** `フィールド干渉>エネルギーつけかえ` の条件を相手の場のエネルギー操作のみに絞り込み、自分の場のつけかえ系を除外する #claude/queue
> [!success] 2026-05-17

#### Phase 37-4: ポケモンいれかえ>味方 からむしよけスプレー除外

`ポケモンいれかえ>味方` に「むしよけスプレー」が不適切に含まれている。パターンマッチ条件を修正して除外する。

- [x] **37-4** `ポケモンいれかえ>味方` から「むしよけスプレー」を除外するようパターン条件を修正する #claude/queue
> [!success] 2026-05-17

#### Phase 37-5: ポケモンのどうぐ独自タグ → カード種別参照>ポケモンのどうぐ に移行

`ポケモンのどうぐ>参照`（14件、「ポケモンのどうぐが装着されていることを条件にした効果」）を `カード種別参照>ポケモンのどうぐ`（新設）として移行する。`ポケモンのどうぐ` 独自タグは空になるので削除。`group_categories_proposal.md` 等の管理ドキュメントから旧 `カード種別参照>ポケモンのどうぐ`（Phase 35-6 で廃止済み・144件）の記載をクリーンアップし、新サブタグの定義を追記する。

- [x] **37-5** `ポケモンのどうぐ>参照`（14件）を `カード種別参照>ポケモンのどうぐ`（新設）に移行し、独自タグ `ポケモンのどうぐ` を削除、管理ドキュメントをクリーンアップする #claude/queue
> [!success] 2026-05-17

#### Phase 37-6: ロック サブタグ調整

`ロック>にげられない>ワザ` サブタグを削除する。加えて「エネルギーをつけられない」効果を持つカードを調査し、`ロック>エネルギー` サブタグを新設して分類する。

- [x] **37-6** `ロック>にげられない>ワザ` を削除し、「エネルギーをつけられない」効果を調査して `ロック>エネルギー` サブタグを新設・分類する #claude/queue
> [!success] 2026-05-17

#### Phase 37-7: 進化加速 サブタグ一時停止

`進化加速` のサブタグ分類を一旦停止し、全カードを親タグのみに集約する（「サイド関連」と同方式）。現状のサブタグ内訳と親タグのみにした場合の件数を確認してから実施すること。

- [x] **37-7** `進化加速` のサブタグ分類を停止し、全カードを親タグのみに集約する #claude/queue
> [!success] 2026-05-17


#### Phase 37-8: 逃げる操作 サブタグ廃止

`逃げる操作>コスト無償` は `逃げる操作` と完全一致（40件 = 40件）のため、サブタグを廃止し親タグのみに統合。

- [x] **37-8** `逃げる操作>コスト無償` サブタグを廃止し、親タグ `逃げる操作`（40件）のみに統合する #claude/queue
> [!success] 2026-05-17

#### Phase 37-9: 特殊状態タグ 付与系・参照系分類・複合付与修正

`特殊状態>どく` 等の付与系タグが「AとBにする」複合形・「A・B・Cの中から」選択形をキャッチできていなかった漏れを修正。
参照系（`特殊状態>参照`）の検出パターンを「特殊状態なら」の汎用参照から個別条件（「どくなら」「どくのポケモン」等）に拡張。
めまいの谷・危険な密林を付与系から参照系に移動。くさりもち・ホミカの演奏・ブーバーン等の参照漏れを修正。

- [x] **37-9** 特殊状態タグを付与系・参照系に分類し、「危険な密林」等を参照に移動、「くさりもち」等のマッチ漏れを修正する
> [!success] 2026-05-17 — 複合付与パターン（どくとX/やけどとX/X・こんらん）追加。参照パターン（どくなら/どくのポケモン）追加。めまいの谷・危険な密林を特殊状態>参照に移動。どく:59→85 / やけど:43→56 / こんらん:72→83 / 参照:14→42
#### Phase 38-1: variant 表示ロジック改修（PROMOキラ・分子>分母対応）

**問題:** 一部カードで PROMOキラや絵違いキラが優先表示されてしまう。

| カード | 現状表示 | 希望表示 | 判別方法 |
|---|---|---|---|
| ボスの指令 | MC `760/742` | MC `724/742` | 分子>分母（絵違いキラ） |
| リーリエのピッピex | MC `765/742` | MC `287/742` | 分子>分母 |
| チェレン | SV-P `279/SV-P` | SV11W `084/086 U` | official_id 個別除外 |
| ビクティニ(Vフォース) | SV-P `271/SV-P` | MC `107/742` | official_id 個別除外 |

**設計:**
- 分子>分母 は機械判定可能 → 一般ルール（`hasOversizedSetNumber`）
- 残りは現状 2 件のみ → official_id 個別 override（`EXCLUDED_OFFICIAL_IDS`）
- レギュ現役判定は導入しない（ポケセンお姉さん型を巻き込まないため）

**実装方針:**
1. `frontend/lib/variantUtils.ts` に `hasOversizedSetNumber(setNumber: string): boolean` を追加（`"X/Y"` 形式かつ X>Y の数値比較で true、分母が数値化不能なら false）
2. `EXCLUDED_OFFICIAL_IDS = new Set(["47716", "49619"])` を定義（各エントリに「除外する PROMOキラ」と「結果として表示したい variant」をコメント記載）
3. `pickDisplayVariant` を「`EXCLUDED_OFFICIAL_IDS` / `SPECIAL_RARITY` / 分子>分母 を除外した残りの先頭、空なら既存フォールバック (`SPECIAL_RARITY` 以外の先頭 ?? `variants[0]`)」に変更

**確認:** `npm run build` が通ること。データ変更なしなのでリビルド不要。

**Out of scope:** ポケセンお姉さん型（PROMO残す挙動）は現状維持。

- [x] **38-1** `frontend/lib/variantUtils.ts` の `pickDisplayVariant` を改修。(1) `hasOversizedSetNumber(setNumber: string): boolean` ヘルパーを追加：`set_number` が `"X/Y"` 形式かつ X>Y の数値比較で true、分母が数値化不能なら false。(2) `const EXCLUDED_OFFICIAL_IDS = new Set(["47716", "49619"])` を定義。47716=チェレン SV-P 279/SV-P PROMOキラを除外し SV11W 084/086 U を表示する意図、49619=ビクティニ(Vフォース) SV-P 271/SV-P PROMOキラを除外し MC 107/742 を表示する意図——両方ともコメントで意図を残す。(3) `pickDisplayVariant` のロジックを「variants から `EXCLUDED_OFFICIAL_IDS` と `SPECIAL_RARITY` と `hasOversizedSetNumber` を除外した残りの先頭を返す、空なら既存フォールバック（`SPECIAL_RARITY` 以外の先頭 ?? `variants[0]`）」に変更。`CardVariant` 型はそのまま使用。`npm run build` で型と Next ビルドが通ることを確認した上で commit & push。 #claude/queue
> [!success] 2026-05-17 — 7[r8[?25h[?25l[?2004h[?1004h[?2031h]0;✳ Claude Code╭───[1CClaude[1CCode[1Cv2.1.143[1C──────────────────...

#### Phase 38-2a: 弱点・抵抗力フィルター UI を削除（内部ロジック維持）

`frontend/app/components/CardSearch.tsx` から弱点・抵抗力フィルターの UI のみを削除。内部の state とフィルタロジックは将来復活できるよう温存する。

- `weaknessFilter` / `resistanceFilter` の `<select>` 要素・ラベル・grid セルを JSX から削除
- 残った grid（にげる プルダウンのみ）の grid-cols 数を適切に調整（`sm:grid-cols-3` → 1要素なので別レイアウトに or 後続 38-2b で にげるをチップ化するので一旦そのままでも可）
- state（`weaknessFilter`, `resistanceFilter`）、`useMemo` のフィルタロジック・依存配列、「全解除」ボタンの `setWeaknessFilter('')`/`setResistanceFilter('')` 呼び出しは**触らない**

- [x] **38-2a** `frontend/app/components/CardSearch.tsx` から弱点（weaknessFilter）と抵抗力（resistanceFilter）の `<select>` UI（ラベル含む、grid セル単位で）を削除する。state、useMemo のフィルタロジック・依存配列、全解除ボタンの `setWeaknessFilter('')`/`setResistanceFilter('')` 呼び出しは内部機能維持のため触らない。にげる プルダウンは残す（38-2b で別途処理）。`npm run build` を通して commit & push。 #claude/queue
> [!success] 2026-05-17 — 7[r8[?25h[?25l[?2004h[?1004h[?2031h]0;✳ Claude Code▗[1C▗[3C▖[1C▖[2CClaude[1CCode[1Cv2.1.143

#### Phase 38-2b: にげる・ワザのエネルギーをチップ化、ワザコスト「個」削除

`frontend/app/components/CardSearch.tsx` の以下のプルダウンをチップ群に置き換える。

- **にげる**: プルダウン → チップ `0/1/2/3/4+` の5チップ、単一選択。`4+` 選択時はフィルタロジックを `retreat >= 4` 判定に変更（既存 `retreatFilter` state を再利用、`'4+'` 等の sentinel 文字列でも、`>=4` 専用フラグでもよい）
- **ワザのエネルギー**: プルダウン → チップ（既存タイプチップと同じ並び・同じ色、ラベル「ワザのエネルギー」で区別）。単一選択、`costTypeFilter` state を再利用
- **ワザコスト数**: チップの「個」表記を削除（`5+` は維持、`0` `1` `2` `3` `4` `5+` の表示に）

- [x] **38-2b** `frontend/app/components/CardSearch.tsx` で以下を実施: (1) 「にげる」プルダウンをチップ群（0/1/2/3/`4+` の単一選択）に置き換え、`4+` 選択時のフィルタロジックを `retreat >= 4` 判定に変更（`retreatFilter` state は再利用、sentinel 文字列でも専用フラグでもよい）。(2) 「ワザのエネルギー」プルダウンをチップ群に置き換え（既存タイプチップと同じ並び・同じ色、ラベル「ワザのエネルギー」で区別、単一選択、`costTypeFilter` state 再利用）。(3) 「ワザコスト数」チップの「個」表記を削除（`5+` は維持、`0`〜`5+` の表示）。`npm run build` を通して commit & push。 #claude/queue
> [!success] 2026-05-17 — 7[r8[?25h[?25l[?2004h[?1004h[?2031h]0;✳ Claude Code▗[1C▗[3C▖[1C▖[2CClaude[1CCode[1Cv2.1.143

#### Phase 38-2c: tagColors.ts でルール+形態テーマを「特別なカード」にマージ

`frontend/lib/tagColors.ts` の `OFFICIAL_CLASS_GROUPS` を編集。

- 「ルール」グループを削除し、「形態テーマ」グループのラベルを「**特別なカード**」にリネーム
- 「特別なカード」の `tags` を `['ポケモンex', 'メガシンカex', 'ACE SPEC', 'テラスタル', '古代', '未来']` に拡張
- 色は purple のまま（既存「形態テーマ」色を継承）
- 「カード種別」グループは**そのまま維持**（特殊エネルギーも残す。フィルタチップから除外するのは 38-2d 側、ここでは色付け用に保持）
- 副作用: `CardModal.tsx` の `getOfficialTagColor` 経由で `ポケモンex`/`メガシンカex`/`ACE SPEC` の表示色が amber → purple に変わる（仕様通り）
- `CardSearch.tsx` の OFFICIAL_CLASS_GROUPS ループは変更不要（結果が自動で変わる）

- [x] **38-2c** `frontend/lib/tagColors.ts` の `OFFICIAL_CLASS_GROUPS` を編集。「ルール」グループを削除し、「形態テーマ」をラベル「特別なカード」にリネーム、tags を `['ポケモンex', 'メガシンカex', 'ACE SPEC', 'テラスタル', '古代', '未来']` に拡張、色は purple のまま継承。「カード種別」グループは特殊エネルギーも含めそのまま維持（フィルタチップから除外する処理は 38-2d 側で行う）。`CardSearch.tsx` の OFFICIAL_CLASS_GROUPS ループ部分は変更不要（自動で結果が反映される）。`CardModal.tsx` の getOfficialTagColor 経由で ポケモンex/メガシンカex/ACE SPEC の色が amber → purple に変わるが仕様通り。`npm run build` を通して commit & push。 #claude/queue
> [!success] 2026-05-17 — 7[r8[?25h[?25l[?2004h[?1004h[?2031h]0;✳ Claude Code╭───[1CClaude[1CCode[1Cv2.1.143[1C──────────────────...

#### Phase 38-2d: カテゴリ + カード種別 を親子並列マージ

`frontend/app/components/CardSearch.tsx` でカテゴリチップ群と OFFICIAL_CLASS_GROUPS の「カード種別」グループ表示を 1 つに統合する。**構造的な変更を伴うため model:opus。**

実装方針:
- CardSearch.tsx 内に `CARD_KIND_OPTIONS` 配列を定義。各エントリは `{ kind: 'category' | 'final_evo' | 'official_tag', value/tag: string, label: string }` の形
- 並び順（親子並列）:
  1. `{kind: 'category', value: 'pokemon', label: 'ポケモン'}`
  2. `{kind: 'final_evo', label: '最終進化のみ'}`
  3. `{kind: 'official_tag', tag: 'たねポケモン', label: 'たねポケモン'}`
  4. `{kind: 'official_tag', tag: '1進化ポケモン', label: '1進化ポケモン'}`
  5. `{kind: 'official_tag', tag: '2進化ポケモン', label: '2進化ポケモン'}`
  6. `{kind: 'official_tag', tag: 'ルールを持つポケモン', label: 'ルールを持つポケモン'}`
  7. `{kind: 'official_tag', tag: 'ワザマシン', label: 'ワザマシン'}`
  8. `{kind: 'category', value: 'グッズ', label: 'グッズ'}`
  9. `{kind: 'category', value: 'サポート', label: 'サポート'}`
  10. `{kind: 'category', value: 'スタジアム', label: 'スタジアム'}`
  11. `{kind: 'category', value: 'ポケモンのどうぐ', label: 'ポケモンのどうぐ'}`
  12. `{kind: 'category', value: 'energy', label: 'エネルギー'}`
- 既存の「カテゴリ」チップ群（628行付近）を、`CARD_KIND_OPTIONS` ベースのチップ群に置き換え
- ラベルは「**カード種別**」
- クリック時:
  - `kind: 'category'` → `setCategoryFilter(value === categoryFilter ? '' : value)`
  - `kind: 'final_evo'` → `setFinalEvoOnly(prev => !prev)`
  - `kind: 'official_tag'` → `toggleOfficialTag(tag)`
- 選択状態判定も同様に kind で分岐
- 色: 「カード種別」グループの sky 色（`OFFICIAL_CLASS_GROUPS` の「カード種別」グループの token を再利用してもよい）で全チップ統一。`finalEvoOnly` が ON の場合も同じ sky 色でハイライト
- `OFFICIAL_CLASS_GROUPS.map` のループでは、`group.label === 'カード種別'` をスキップ（残り 2グループ＝特別なカード・キャラクターのみループ）
- **特殊エネルギー**チップは `CARD_KIND_OPTIONS` に含めない（`category: energy` チップで代替されるため）

- [x] **38-2d** `frontend/app/components/CardSearch.tsx` でカテゴリと「カード種別」を 1 グループにマージする。(1) CardSearch.tsx 内に `CARD_KIND_OPTIONS` 配列を定義: 各エントリ `{kind: 'category'|'final_evo'|'official_tag', value/tag, label}`、並び順は ポケモン(category=pokemon)・最終進化のみ(final_evo)・たねポケモン(official_tag)・1進化ポケモン・2進化ポケモン・ルールを持つポケモン・ワザマシン・グッズ(category)・サポート・スタジアム・ポケモンのどうぐ・エネルギー(category=energy)。(2) 既存「カテゴリ」チップ群を `CARD_KIND_OPTIONS` ベースに置換、ラベルは「カード種別」。クリックハンドラを kind で振り分け（category→setCategoryFilter、final_evo→setFinalEvoOnly、official_tag→toggleOfficialTag）、選択状態も kind で分岐。色は sky で統一。(3) OFFICIAL_CLASS_GROUPS.map のループで `group.label === 'カード種別'` をスキップ（残り 2 グループ＝特別なカード・キャラクターのみループ）。(4) 特殊エネルギーは `CARD_KIND_OPTIONS` に含めない（energy チップで代替）。`npm run build` を通して commit & push。 model:opus #claude/queue
> [!success] 2026-05-17 — 7[r8[?25h[?25l[?2004h[?1004h[?2031h]0;✳ Claude Code▗[1C▗[3C▖[1C▖[2CClaude[1CCode[1Cv2.1.143

#### Phase 38-2e: 表示順並べ替え

`frontend/app/components/CardSearch.tsx` の公式フィルター Panel 内 JSX 並び順を以下に並び替える（**38-2a〜2d 完了後の状態を前提**）。

```
1. カード種別（38-2d でマージ後）
2. タイプ
3. 特別なカード（38-2c でマージ後）
4. キャラクター
5. HP スライダー
6. ワザのエネルギー + ワザコスト数（横並び維持）
7. にげる（38-2b でチップ化済み）
```

- [x] **38-2e** `frontend/app/components/CardSearch.tsx` の公式フィルター Panel 内 JSX 並び順を以下に並び替える: (1) カード種別（38-2d マージ後）→ (2) タイプ → (3) 特別なカード（38-2c マージ後、OFFICIAL_CLASS_GROUPS ループの該当グループ）→ (4) キャラクター（OFFICIAL_CLASS_GROUPS の該当グループ）→ (5) HP スライダー → (6) ワザのエネルギー + ワザコスト数（横並び維持）→ (7) にげる（38-2b チップ化後）。state やロジックの変更は不要、JSX 順序の入れ替えのみ。`npm run build` を通して commit & push。 #claude/queue
> [!success] 2026-05-17 — 7[r8[?25h[?25l[?2004h[?1004h[?2031h]0;✳ Claude Code▗[1C▗[3C▖[1C▖[2CClaude[1CCode[1Cv2.1.143


#### Phase 38-3: NAMED_OVERRIDES の仕組み追加 + ハンディサーキュレーター/ヒートバーナー再分類

`scripts/tag_cards.mjs` にカード名ベースのタグ上書き機構を追加する。Q&A 側 (`tag_qa_entries.mjs`) と同じく「正規表現で拾い切れない特殊効果」に対応するための先行実装。**今回は仕組み追加 + 既知 2 件のエントリのみ。** 残りの洗い出しは Phase 33（軽減・無効）完了後に別タスクで実施予定。

**実装内容:**

1. `scripts/tag_cards.mjs` の `TAG_RULES` 配列の後・`assignTags` 関数の前に `NAMED_OVERRIDES` 定数を追加:
...
2. `assignTags` 関数の末尾（`return [...tags].sort()` の直前）に override 適用ロジックを追加:
...
3. リビルド順: `node scripts/tag_cards.mjs` → `node scripts/tag_qa_entries.mjs` → `node scripts/build_qa_index.mjs`

4. 動作確認:
   - `data/card_tags.json` で `ハンディサーキュレーター` のタグから `エネ加速` 系が消え、`フィールド干渉`/`フィールド干渉>エネルギーつけかえ` が付いていること（`カウンター効果（ダメカン以外）` は維持）
   - `data/card_tags.json` で `ヒートバーナー` のタグから `カード種別参照*` と `条件ダメージ*` が全て消え、`フィールド干渉`/`フィールド干渉>どうぐトラッシュ`/`フィールド干渉>エネルギートラッシュ`/`フィールド干渉>スタジアムトラッシュ` が付いていること

5. `npm run build` でビルド確認 → commit & push

**Out of scope:** `tag_qa_entries.mjs` 側の override 機構は今回追加しない（Q&A はテキストベースで判定するため別概念）。

- [x] **38-3** `scripts/tag_cards.mjs` にカード名ベースのタグ上書き機構 `NAMED_OVERRIDES` を追加。(1) `TAG_RULES` 配列の後・`assignTags` 関数の前に `const NAMED_OVERRIDES = { 'ハンディサーキュレーター': { add: ['フィールド干渉', 'フィールド干渉>エネルギーつけかえ'], remove: ['エネ加速', 'エネ加速>つけかえ'] }, 'ヒートバーナー': { add: ['フィールド干渉', 'フィールド干渉>どうぐトラッシュ', 'フィールド干渉>エネルギートラッシュ', 'フィールド干渉>スタジアムトラッシュ'], remove: ['カード種別参照', 'カード種別参照>特殊エネルギー', '条件ダメージ', '条件ダメージ>エネルギー消費：手札'] } };` を定義（コメントで「パターンマッチで拾えない特殊効果カード用」「全 regex ルール適用後に上書き」と説明）。(2) `assignTags` の `return [...tags].sort()` の直前に `const ov = NAMED_OVERRIDES[c.name]; if (ov) { ov.remove?.forEach(t => tags.delete(t)); ov.add?.forEach(t => tags.add(t)); }` を追加。(3) リビルド: `node scripts/tag_cards.mjs` → `node scripts/tag_qa_entries.mjs` → `node scripts/build_qa_index.mjs`。(4) `data/card_tags.json` で ハンディサーキュレーター が `エネ加速`/`エネ加速>つけかえ` を持たず `フィールド干渉`/`フィールド干渉>エネルギーつけかえ` を持つこと（`カウンター効果（ダメカン以外）` は維持）、ヒートバーナー が `カード種別参照*`/`条件ダメージ*` を全て持たず `フィールド干渉`/`フィールド干渉>どうぐトラッシュ`/`フィールド干渉>エネルギートラッシュ`/`フィールド干渉>スタジアムトラッシュ` を持つことを確認。`npm run build` でビルド確認 → commit & push。 #claude/queue
> [!success] 2026-05-19 — Gardener が直接実装（2026-05-17 の Worker 失敗を受けて再投入を見送り）。tag_cards.mjs に NAMED_OVERRIDES 機構を追加。ハンディサーキュレーター → フィールド干渉>エネルギーつけかえ、ヒートバーナー → フィールド干渉>{どうぐ/エネルギー/スタジアム}トラッシュ で動作確認。`npm run build` ✅。commit `4dfc650` push 済み。
#### Phase 33-A: 基盤整備 — 新軸②6値 + 耐性親タグ + 5サブを GROUP_ORDER に追加（出力差分なし）

`scripts/tag_cards.mjs` の `GROUP_ORDER` 配列と `SECTION_MAP` オブジェクトに、Phase 33 新タグの「場所だけ」を予約する。判定ロジックは追加しない（後続タスク 33-B〜F で実装）。新タグはどのカードにも付与されないため、`data/card_tags.json` と `docs/tagging_stats.md` の出力に差分が出ないことが完了基準。旧タグ（ベンチ狙撃 / ダメカン操作 / ダメージ操作 / 無効 / 軽減 / 特殊状態>耐性）は **温存**（後続タスクで判定ロジックと一緒に削除する）。
...
**配置位置**: `GROUP_ORDER` 配列内、既存のダメージ系タグ群（ベンチ狙撃 / ダメカン操作 / ダメージ操作 等）の **直前** に、コメント `// ── Phase 33 新軸② 親タグ群（判定ロジックは 33-B〜F で実装、現段階では予約のみ） ──` を入れて差し込む。`SECTION_MAP` には `'§Phase33'` セクション ID を新設し、12タグ全てをマップする。

**動作確認**:
1. `node scripts/tag_cards.mjs` を実行
2. 実行後の `data/card_tags.json` を実行前と比較し **完全一致** であること（git diff が空）
3. `docs/tagging_stats.md` の「各グループ検出数」表に新タグ12個が登場するが、件数列はすべて空欄 or 行自体が表示されない（既存ロジック `if (!(tag in tagCounts)) continue;` により未付与タグはスキップされる）。`docs/tagging_stats.md` の git diff も空であることを確認
4. Q&A 側は変更不要（タグ判定対象ではないため `tag_qa_entries.mjs` の再実行のみで OK、出力差分なし）

**スコープ外**:
- 軸④タイミング系タグ（ワザ使用時>持続効果・常時>条件付き・トリガー型>きぜつ等）の追加は 33-F に持ち越し（命名規則を実装フェーズで再確認するため）
- 既存タグの削除は後続タスク

- [x] **33-A** `scripts/tag_cards.mjs` の `GROUP_ORDER` 配列にコメント `// ── Phase 33 新軸② 親タグ群（判定ロジックは 33-B〜F で実装、現段階では予約のみ） ──` を入れた上で12個の新タグ（`'ワザダメージ', 'ダメカン直置き', '与ダメージ修飾', '受けるダメージ軽減', '受けるダメージ無効', 'ダメカン移動', '耐性', '耐性>ワザの効果', '耐性>グッズの効果', '耐性>サポートの効果', '耐性>ベンチ効果', '耐性>特殊状態'`）を、既存ダメージ系タグ群（ベンチ狙撃 / ダメカン操作 / ダメージ操作）の直前に追加する。`SECTION_MAP` には新セクション ID `'§Phase33'` を新設し、12タグ全てにマップを追加する。判定ロジック（TAG_RULES への push）は **絶対に追加しない**。旧タグ（ベンチ狙撃 / ダメカン操作 / ダメージ操作 / 無効 / 軽減 / 特殊状態>耐性）も **絶対に削除しない**。完了後 `node scripts/tag_cards.mjs` → `node scripts/tag_qa_entries.mjs` → `node scripts/build_qa_index.mjs` の順でリビルド。`data/card_tags.json` と `docs/tagging_stats.md` の git diff が空であることを `git diff --stat data/card_tags.json docs/tagging_stats.md` で確認（差分がある場合は実装ミス。差分内容を報告して失敗扱い）。最後に `cd frontend && npm run build` でビルド成功確認 → 親リポジトリで commit & push。 #claude/queue
> [!success] 2026-05-18 — 7[r8[?25h[?25l[?2004h[?1004h[?2031h]0;✳ Claude Code╭───[1CClaude[1CCode[1Cv2.1.143[1C──────────────────...


#### Phase 33-B: 軸②値 #1 ワザダメージ + #2 ダメカン直置き + #6 ダメカン移動 + 軸④親タグ「ワザを受けたとき」を実装、旧「ベンチ狙撃」「ダメカン操作」を解体

`scripts/tag_cards.mjs` で旧§4 (ベンチ狙撃) と §B-4 (ダメカン操作) の TAG_RULES エントリ群を全削除し、TAG_RULES 末尾に新セクション §Phase33-1〜4 を追加。軸③は軸②値親タグのサブとして限定表現 (案B1)、軸④は独立親タグで並列付与 (案A2)、反射は §Phase33-4 で「ダメカン直置き + ワザを受けたとき」の2タグ並列 (案C2)。`GROUP_ORDER` から旧8タグ削除＋新サブ挿入＋`ワザを受けたとき` を耐性親タグ群の前に独立配置。`SECTION_MAP` も同様に同期。

- [x] **33-B** §Phase33-1〜4 を TAG_RULES 末尾に追加、旧§4/§B-4 削除、GROUP_ORDER/SECTION_MAP 同期。
> [!success] 2026-05-19 — Gardener が直接実装（Worker silent fail を受けて再投入を見送り）。Top tags 検証で旧8タグ完全消滅、新タグ件数:
> - ワザダメージ 158 / ワザダメージ>ベンチ 158 / ワザダメージ>全体スプレッド 23 (期待値ピッタリ)
> - ダメカン直置き 109 (期待 ~140 だが旧 `ダメカン操作 116` から `のせ替える` 20件が `ダメカン移動` に正しく分離した結果。設計通り)
> - ダメカン直置き>ベンチ 48 / ダメカン移動 20 / ワザを受けたとき 33
>
> 代表カード検証: マリィのオーロンゲex / ヨノワール / 危ない廃墟 / スパイクエネルギー OK。ドラパルトex は spec の期待 `ワザダメージ` が design ロジック (BENCH_BRACKET 必須) と矛盾、設計通り `ダメカン直置き+ダメカン直置き>ベンチ` のみ付与（spec 記述ミス）。`npm run build` ✅。
