...
### 後-A: 基本エネルギー言及カードのタグ漏れ補完 ✅
- 対象: テキストに「基本エネルギー」を含むが適切なエネルギー系タグを持たないカード (全5246枚中142件/36ユニーク名)
- `scripts/tag_cards.mjs` の6箇所を修正:
  - §5-A (エネ加速>山札→手札): 「その中から」パターン追加
  - §5-B (エネ加速>山札→ポケモン): 「ポケモン1匹に」「ずつつける」パターン追加
  - §B-1-T2 (トラッシュ回収>エネルギー): 旧式テキスト・「てよい」終端・ハンテール等を追加
  - §B-5-a (エネ移動>自分・場のエネ自由配分): 「基本エネルギー」の鍵括弧ありパターン追加
  - §B-6-2 (エネ除去>自己コスト): 大幅拡張 (42→217件); カイオーガ/セキタンザン/ナンジャモのタイカイデン等を捕捉
  - §B-6-2 (エネ除去>自己コスト): ソウブレイズの誤検知修正 (「相手のポケモン全員についている特殊エネルギーを、すべてトラッシュする」を除外)
- 残り7ユニーク名は意図的スキップ: モグリュー/ヤバソチャex/マホイップ/パルキア/メガニウム/メガユキノオーex/クレベース (既存タグに非対応の効果)
- 最終タグ数: エネ加速>山札→手札 89, エネ加速>山札→ポケモン 101, エネ移動>自分・場のエネ自由配分 46, トラッシュ回収>エネルギー 131, エネ除去>自己コスト 217
