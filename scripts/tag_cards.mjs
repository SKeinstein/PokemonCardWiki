...
const TAG_RULES = [
  ...
  {
    tags: ['自傷', '自傷>ダメージ'],
    condition: (all) => /このポケモンにも.{1,20}ダメージ/.test(all),
  },
  ...
  // 古びたはねの化石 (はねのまもり): ベンチにいるかぎりワザのダメージを受けない
  {
    ...
    condition: (all, atk, abl, name, rul) =>
      // Stadium rules: bench can't have ダメカン placed (バトルコロシアム)
      /ベンチポケモン.{1,60}ダメカンがのらない/.test(rul) ||
      // Stadium rules: bench takes no attack damage
      /ベンチポケモン.{1,60}ワザのダメージを受けない/.test(rul) ||
      // Ability: bench-wide damage protection (damage only, not effects)
      (/ベンチポケモン.{1,60}ワザのダメージを受けない/.test(abl) && !/ワザのダメージや効果/.test(abl)) ||
  ...
  // カスミのコイキング (すごいもぐる): 同上
  {
    ...
    condition: (all, atk, abl) =>
      /ベンチポケモン.{1,60}ワザのダメージや効果を受けない/.test(abl) ||
      /ベンチにいるかぎり.{1,60}ワザのダメージや効果を受けない/.test(abl),
  },
...
const SECTION_MAP = {
  ...
  'ワザロック':         '§B-7','グッズサポートロック': '§B-8','軽減': '§B-9',
