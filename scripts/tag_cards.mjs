...
const TAG_RULES = [
  ...
  // ── §B-17  条件付きダメージ強化 ──────────────────────────────────────────
  // §B-17-1a  +N条件加算型 → 与ダメージ修飾>条件付き加算
  //   Phase 33-K セッション2 (2026-05-24): 設計書 §9-13「(b) +N条件加算は『与ダメージ修飾』と意味同一」に基づき、
  //   条件ダメージ系から (b) パターンを横断的に拾うため広い regex に統合。
  //   §B-17-2 / §B-17-2-additive / §B-17-3 から (b) パターンを本ルールに移管 (該当行は削除済)。
  //   "の数×N" / "枚数×N" は条件節 (なら/とき/いれば) を持たないため自然に除外される。
  {
    tags: ['与ダメージ修飾', '与ダメージ修飾>条件付き加算'],
    condition: (all) =>
      // 広めルール: 条件節 + Nダメージ追加 型を網羅 (ex/V/特殊状態/進化/コイン1回/エネ条件/サイド/手札/前の番/etc.)
      /(?:なら|とき|いれば)、?\s*\d+ダメージ追加/.test(all) ||
      // "残りHP …なら …ダメージ" (「ダメージ追加」suffix がない形式の救済)
      /残りHP.{1,20}ダメージ/.test(all) ||
      // "自分のベンチに、名前に「〇〇」とつくポケモンがいるなら" (続きが ダメージ追加 以外でも +N意図 — Phase 33-C 由来、互換のため温存)
  ...
  // §B-17-2  相手の状態・エネ参照 (×N可変・spread系のみ)
  //   Phase 33-K セッション2: (b) +N加算ルール (特殊状態なら / ex/V/テラスタルなら) は §B-17-1a に移管 (削除)。
  //   本ルールは ×N可変 と spread (相手全員に〜) のみ残す。
  {
    tags: ['条件ダメージ', '条件ダメージ>相手参照'],
    condition: (all) =>
      // Opponent energy total ×N
      /相手.{0,20}エネルギー.{0,15}の数[×x]/.test(all) ||
      /相手のポケモン全員.{0,20}エネルギー.{0,20}ダメージ/.test(all) ||
      /エネルギー.{1,10}合計.{1,10}ダメージ/.test(all) ||
      // Opponent ex count ×N / spread to ex
      /相手の場の「ポケモンex」の数[×x]/.test(all) ||
      /相手の「ポケモンex」全員に.{1,20}ダメージ/.test(all) ||
      // グッズ/サポートの枚数×ダメージ (議題3-B: 相手 prefix 必須)
      matchOppClause(all, /グッズの枚数[×x]\d+ダメージ/) ||
      matchOppClause(all, /サポートの枚数[×x]\d+ダメージ/),
  },
  // §B-17-3  コイン可変ダメージ (×N可変のみ)
  //   Phase 33-K セッション2: 単発コイン +N加算 (`コインを1回投げオモテなら、\d+ダメージ追加`) は §B-17-1a に移管 (削除)。
  {
  ...
  // Phase 33-K: 旧「攻撃>ワザを受けたとき」を「ワザを受けたとき」に改名（プレフィックス廃止）
  // 反射カードは「ダメカン直置き>相手バトル」(上記§B-4a)+「ワザを受けたとき」の並列
  {
    tags: ['ワザを受けたとき'],
...
// Phase 33-J: カードの軸② parent 集合から `攻撃` / `防御` 親タグを直接付与し、
//   軸④ (常時/番の終わり/トリガー型/ワザ使用時>持続効果) を source block
//   (atk / abl / rul) と passive 内容のキーワード辞書で 攻撃 / 防御 を attribute する。
// Phase 33-K: 「攻撃」親タグ廃止、攻撃側タイミング系サブをトップレベル親タグに昇格。
//   防御側は温存（防御セクション再編は将来タスク）。
//
// 出力タグ:
//   - 攻撃側: `常時>{無条件|条件付き}` / `番の終わり` / `トリガー型>{起動型|きぜつ|その他}` / `ワザ使用時>持続効果` / `ワザを受けたとき`
//   - 防御側: `防御>常時>{無条件|条件付き}` 等（プレフィックス温存）
...
// Phase 33-K: 攻撃側はプレフィックスなしでタグ出力、防御側は `防御>` プレフィックスを温存。
function emitSideTag(side, suffix) {
  return side === '攻撃' ? suffix : `${side}>${suffix}`;
}

function deriveAxis4Tags(c, tags) {
  const hasAttack =
    [...tags].some(t => ATTACK_PARENTS.has(t)) ||
    tags.has('ワザを受けたとき'); // §Phase33-4 反射 (Phase 33-K 改名)
  const hasDefense = [...tags].some(t => DEFENSE_PARENTS.has(t));
  if (!hasAttack && !hasDefense) return;

  // Phase 33-K: 「攻撃」親タグは廃止。防御は温存。
  ...
  if (endHits.atk || endHits.abl || endHits.rul) {
    for (const side of attribute(endHits)) {
      tags.add(emitSideTag(side, '番の終わり'));
    }
  }
  ...
  if (PERSIST_PAT.test(atkRaw)) {
    for (const side of attribute({ atk: true, abl: false, rul: false })) {
      tags.add(emitSideTag(side, 'ワザ使用時>持続効果'));
    }
  }
  ...
  if (trigger) {
    for (const side of attribute(trigHits)) {
      tags.add(emitSideTag(side, 'トリガー型'));
      tags.add(emitSideTag(side, `トリガー型>${trigger}`));
    }
  }
  ...
  if (ablIsPassive || rulIsPassive) {
    ...
    for (const side of attribute(passiveHits)) {
      tags.add(emitSideTag(side, '常時'));
      tags.add(emitSideTag(side, `常時>${subkey}`));
    }
  }
}
...
const GROUP_ORDER = [
  ...
  // ── Phase 33-K: ダメージ系（攻撃）セクション親タグ群（「攻撃」親タグ廃止＋タイミング系昇格＋条件ダメージ統合） ──
  // メカニズム系（軸②）
  ...
  'ダメカン移動',
  // タイミング系（旧 攻撃>X からトップレベル親タグに昇格）
  '常時', '常時>無条件', '常時>条件付き',
  'トリガー型', 'トリガー型>起動型', 'トリガー型>きぜつ', 'トリガー型>その他',
  'ワザ使用時>持続効果',
  'ワザを受けたとき',
  '番の終わり',
  // 条件ダメージ (Phase 33-K: 防御セクションから攻撃セクションへ移管。サブは温存)
  '条件ダメージ', '条件ダメージ>場参照', '条件ダメージ>相手参照', '条件ダメージ>コイン可変', '条件ダメージ>自傷', '条件ダメージ>エネルギー消費：手札',
  // フォールバック (Phase 33-K: 判断迷い吸収用。場参照(b) 移行は次フェーズ)
  '未分類',
...
const SECTION_MAP = {
  ...
  'ダメカン移動': '§Phase33',
  // Phase 33-K: タイミング系親タグ（旧 攻撃>X からトップレベル昇格）
  '常時': '§Phase33-K', '常時>無条件': '§Phase33-K', '常時>条件付き': '§Phase33-K',
  'トリガー型': '§Phase33-K', 'トリガー型>起動型': '§Phase33-K',
  'トリガー型>きぜつ': '§Phase33-K', 'トリガー型>その他': '§Phase33-K',
  'ワザ使用時': '§Phase33-K', 'ワザ使用時>持続効果': '§Phase33-K',
  'ワザを受けたとき': '§Phase33-K',
  '番の終わり': '§Phase33-K',
  // Phase 33-K: 条件ダメージは攻撃セクションに移管（タグ自体は §B-17 で生成）
  '条件ダメージ': '§B-17', '条件ダメージ>場参照': '§B-17', '条件ダメージ>相手参照': '§B-17',
  '条件ダメージ>コイン可変': '§B-17', '条件ダメージ>自傷': '§B-17', '条件ダメージ>エネルギー消費：手札': '§B-17',
  // Phase 33-K: 判断迷い吸収用フォールバック
  '未分類': '§Phase33-K',
  '防御': '§Phase33-J',
