...
const TAG_RULES = [
  ...
  // NOTE: patterns require 受けない to avoid matching "受けてきぜつ" (メガゲンガーex, ヌケニン)
  {
    tags: ['無効', '無効>条件付きその他'],
    condition: (all, atk, abl) =>
      /「ポケモンex」から.{0,10}ワザのダメージ.{0,10}受けない/.test(abl) ||
      /「ポケモンV」から.{0,10}ワザのダメージ.{0,10}受けない/.test(abl) ||
      /特性を持つポケモンからワザのダメージ.{0,10}受けない/.test(abl) ||
      /「\d+」以上のワザのダメージを受けない/.test(abl) ||
      /特殊エネルギーがついている.{1,20}ワザのダメージ.{0,10}受けない/.test(abl) ||
      /「テラスタル」のポケモンから.{0,10}ワザのダメージ.{0,10}受けない/.test(abl),
  },
