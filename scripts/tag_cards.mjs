...
const TAG_RULES = [
  ...
  // 「のぞむなら」型 = プレイヤーが任意で効果発動するか選べる構文。
  // QA結合用のみ・検索UIには出さない (frontend/lib/tagColors.ts の RULE_TERMS_TAGS で除外)。
  {
    tags: ['ルール用語', 'ルール用語>のぞむなら'],
    condition: (all) =>
      /のぞむなら/.test(all) ||
      /のぞまないなら/.test(all) ||
      /のぞむか[、,]?.{0,15}たずねる/.test(all),
  },
...
const SECTION_MAP = {
  ...
  'ベンチに届く': '§P33-M', '自分側': '§P33-M', 'お互い': '§P33-M', 'バトル場のみ': '§P33-M',
