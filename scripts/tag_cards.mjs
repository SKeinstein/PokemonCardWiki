...
const TAG_RULES = [
  ...
  // §B-1-T2  トラッシュ回収>エネルギー: トラッシュからエネルギーを手札 or ポケモン or 山札へ
  //   エネ加速>トラッシュ→ポケモン と意図的に重複付与（再利用可能性 vs 直接装着の両側面）
  //   旧テキスト「トラッシュにある基本エネルギー」形式 (エネルギー回収/リサイクル旧版) も捕捉
  //   「ポケモンまたは基本エネルギー」「ポケモンと基本エネルギー」複合サルベージ (夜のタンカ/スイレンのお世話等) も捕捉
  //   「手札に加えてよい」形式 (ハッコウシティ) も `手札に加え` でマッチ
  {
    ...
    condition: (all, atk, abl, name) =>
      ...
      /トラッシュにある.{0,10}エネルギー.{0,30}(?:手札に加え|山札にもどして切る)/.test(all) ||
      /トラッシュから.{1,60}基本エネルギー.{0,50}(?:手札に加え|山札にもどして切る)/.test(all) ||
  ...
  //         （エネルギーつけかえ／ハピナスex／エネコロロ／フォレトス / メガフシギバナex 等）
  //         「基本エネルギー」（鍵かっこあり）形式にも対応
  {
    tags: ['エネ移動', 'エネ移動>自分・場のエネ自由配分'],
    condition: (all, atk, abl, name) =>
      /自分の(場の)?ポケモン(の|について(い[るた])?)「?(基本)?エネルギー」?を.{0,30}自分の(別の)?ポケモン.{0,15}つけ替える/.test(all) ||
  ...
  {
    tags: ['グッズサポートロック', 'グッズサポートロック>トレーナー効果無効'],
    condition: (all) =>
  ...
  },
  {
    tags: ['ACE SPEC参照'],
    condition: (all) => {
      const stripped = all.replace(/ACE SPECのカードは、デッキに1枚しか入れられない。/g, '');
      return /ACE SPEC/.test(stripped);
    },
  },
  {
    tags: ['古代参照'],
...
const GROUP_ORDER = [
  ...
  'グッズサポートロック', 'グッズサポートロック>グッズロック', 'グッズサポートロック>サポートロック', 'グッズサポートロック>ACE SPECロック', 'グッズサポートロック>トレーナー効果無効',
...
const SECTION_MAP = {
  ...
  'エネ除去': '§B-6',
  'ワザロック':         '§B-7','にげロック': '§B-27','にげロック>相手': '§B-27','にげロック>自分': '§B-27','グッズサポートロック': '§B-8','グッズサポートロック>トレーナー効果無効': '§B-8','軽減': '§B-9',
