...
const TAG_RULES = [
  ...
  // — ザシアンex / フリーザー / オーガポン各種 / デオキシス(ゲノムチャージ) 等
  {
    tags: ['エネ加速', 'エネ加速>山札→ポケモン'],
    condition: (all) =>
      /山札から.{1,30}エネルギー.{1,50}ポケモンに.{0,20}つける/.test(all) ||
  ...
  //   — タケルライコex「きょくらいごう」, ゼイユ, ミガルーサex, アオキの手際 等
  {
  ...
  //   NOTE: 「ポケモンのどうぐ」を含む文は除外（ポケモンのどうぐは グッズ サーチへ）
  {
    tags: ['サーチ', 'サーチ>ポケモン'],
    condition: (all, atk, abl, name) => {
  ...
  //   トレーナーズ検索（任意の種類）はサポートにもグッズにも該当するため両方付与
  {
    tags: ['サーチ', 'サーチ>サポート'],
    condition: (all) =>
  ...
  // §B-1-S3  サーチ>グッズ: 山札からグッズ/ポケモンのどうぐ/スタジアムを手札へ
  //   トレーナーズ検索もここに含める（任意の種類を選べるため）
  {
    tags: ['サーチ', 'サーチ>グッズ'],
    condition: (all) =>
  ...
  // §B-1-S4  サーチ>自由選択: 山札から好きなカードを手札へ
  {
    tags: ['サーチ', 'サーチ>自由選択'],
    condition: (all) =>
  ...
  // ── §B-1-T  トラッシュ回収（トラッシュからカードを手札or場に戻す）────────
  // §B-1-T1  トラッシュ回収>ポケモン: トラッシュからポケモンを手札 or ベンチへ
  {
    tags: ['トラッシュ回収', 'トラッシュ回収>ポケモン'],
    condition: (all) =>
      /トラッシュから.{0,15}ポケモン.{0,15}を.{0,15}選び.{0,30}手札に加える/.test(all) ||
      /トラッシュから.{0,15}たねポケモン.{0,30}手札に加える/.test(all) ||
      /トラッシュから.{0,15}ポケモン.{0,30}選び.{0,30}ベンチに出す/.test(all),
  },
  // §B-1-T2  トラッシュ回収>エネルギー: トラッシュからエネルギーを手札 or ポケモンへ
  //   エネ加速>トラッシュ→ポケモン と意図的に重複付与（再利用可能性 vs 直接装着の両側面）
  {
    tags: ['トラッシュ回収', 'トラッシュ回収>エネルギー'],
    condition: (all, atk, abl, name) =>
      /トラッシュから.{0,5}「?(?:基本)?エネルギー」?を.{0,30}選び.{0,30}手札に加える/.test(all) ||
      /トラッシュから.{0,5}「?(?:基本)?エネルギー」?を.{0,30}選び.{0,30}つける/.test(all) ||
      /トラッシュから.{0,15}エネルギーカード.{0,30}手札に加える/.test(all) ||
      name === 'ブーメランエネルギー',
  },
  // §B-1-T3  トラッシュ回収>自由選択: トラッシュから任意のカード/サポート/トレーナーズ
  {
  ...
  // 「おたがいのポケモン全員のHPを」「ポケモン全員のHPを」系も複数回復として捕捉
  // — メガフラエッテex やさしいひかり 等
  {
    tags: ['回復', '回復>複数回復'],
    condition: (all) =>
      /ベンチ.{1,30}HPを.{1,20}回復/.test(all) ||
      /すべての.{1,20}HPを.{1,20}回復/.test(all) ||
      /ポケモン全員.{0,20}HPを.{1,20}回復/.test(all) ||
      /おたがいの.{1,30}HPを.{1,20}回復/.test(all),
  },
  ...
  // ── §B-5  エネルギー移動（つけ替え）─────────────────────────────────────
  // 親タグ：すべての「つけ替える」効果（エネルギーつけかえ系・特性・ワザを含む）
  {
    tags: ['エネ移動'],
    condition: (all, atk, abl, name) =>
      /エネルギーを.{1,30}つけ替える/.test(all) || name === 'エネルギーつけかえ',
  },
  // §B-5-a  自分・場のエネ自由配分：「自分の(場の)ポケモン → 自分の別のポケモン」へ任意配分
  //         （エネルギーつけかえ／ハピナスex／エネコロロ／フォレトス 等）
  {
    tags: ['エネ移動', 'エネ移動>自分・場のエネ自由配分'],
    condition: (all, atk, abl, name) =>
      /自分の(場の)?ポケモン(の|について(い[るた])?)(基本)?エネルギーを.{0,30}自分の(別の)?ポケモン.{0,15}つけ替える/.test(all) ||
      /好きなだけ選び、自分のポケモンに好きなようにつけ替える/.test(all),
  },
  // §B-5-b  自分・集約：自分の場/ベンチ → このポケモン or バトルポケモンへ集める
  //         （Nの筋書き／ジュゴン／テツノイサハex／コバルオンex／マリルリex／ラティオス／スクランブルスイッチ）
  {
    tags: ['エネ移動', 'エネ移動>自分・集約'],
    condition: (all) =>
      /(自分の場のポケモン|自分のベンチポケモン|(ベンチに)?入れ替えたポケモン)について(い[るた])?エネルギーを.{1,30}(このポケモン|バトルポケモン|新しくバトル場に出したポケモン|新しいバトルポケモン)につけ替える/.test(all),
  },
  // §B-5-c  自分・拡散：このポケモン → ベンチポケモン（離脱・退避・拡散）
  //         （テツノワダチ／メガヤンマex／ボルケニオンex／メガゲンガーex／モスノウ／ケルディオ／シェイミ／
  //           テツノイバラex／ポワルン／タイカイデン／ルージュラ／トルネロス／ヘビーバトン）
  {
    tags: ['エネ移動', 'エネ移動>自分・拡散'],
    condition: (all) =>
      /(この|その)ポケモンについて(い[るた])?(基本)?エネルギーを.{1,30}(自分の)?(ベンチポケモン|新しく出したポケモン).{0,15}つけ替える/.test(all),
  },
  // §B-5-d  相手のエネ：相手の場のエネを動かす撹乱（相手の場でつけ替え）
  //         （ゲンガーex／ロケット団のサンダー／ニャオニクス／リグレー／ハンディサーキュレーター）
  {
    tags: ['エネ移動', 'エネ移動>相手のエネ'],
    condition: (all, atk, abl, name) =>
      /相手の.{0,15}ポケモンについて(い[るた])?エネルギーを.{1,30}相手の.{1,15}つけ替える/.test(all) ||
      name === 'ハンディサーキュレーター',
  },
  ...
  // §B-27  にげロック（相手のポケモンをにげられなくする）
  {
    tags: ['にげロック', 'にげロック>相手'],
    condition: (all) => /次の相手の番、このワザを受けたポケモンは、にげられない/.test(all),
  },
  // §B-27  にげロック（自分のポケモンがにげられなくなる）
  {
    tags: ['にげロック', 'にげロック>自分'],
  ...
  // ── §B-13  進化加速 ───────────────────────────────────────────────────────
  // §B-13-1  先攻1番目からの進化（ルール制限解除）
  {
    tags: ['進化加速', '進化加速>ルール制限解除'],
    condition: (all) => /先攻最初の番.{1,30}進化させる/.test(all),
  },
  // §B-13-2  山札/手札からの即時進化（サーチ型）
  {
    tags: ['進化加速', '進化加速>山札から'],
    condition: (all, atk, abl, name) =>
  ...
  {
    tags: ['進化加速', '進化加速>条件進化'],
    condition: (all) => /残りHPが.{1,10}なら.{1,30}進化/.test(all),
  },
  ...
  { tags: ['サイド操作'], condition: (all) => /自分のサイドを\d枚とる/.test(all) },
  ...
  // レガシーエネルギー: §B-1-T3 にて name-based 付与済み（トラッシュ回収>自由選択）

  // ── §B-20  ポケモンチェック参照 ──────────────────────────────────────────
  { tags: ['ポケモンチェック'], condition: (all) => /ポケモンチェックのたびに?/.test(all) },

  // ── §B-21  手札トラッシュ（手札干渉サブカテゴリ）────────────────────────
  // 手札をすべてトラッシュ系（旧 §B-21 から手札干渉サブとして残置）
  // ※ 旧「手札シャッフル」トップレベルタグは §16-1 にて廃止。シャッフル→ドローは
  //   §B-1 ドロー>シャッフル後 へ吸収済み。
  {
  ...
  //   除外: 自己制限「この特性は使えない」系（スピンロトム/キチキギスex/モモワロウex/
  //         ニャースex/ルナトーン/メガガルーラex 等。主体が「相手|おたがい」でないため
  //         自然に外れる）
  ...
  // §B-24-4  与ダメ強化: 「相手の…へのダメージは「+N」される」系（自分の与ダメ buff）
  //   ポケモンのどうぐ・サポート・グッズ・スタジアム・特性で広く使われる定型句。
  //   「へのダメージは「+N」」 (TO 相手) のみ対象とし、「受けるワザのダメージは-N」(被ダメ軽減,
  //   §B-9) や「受けるワザのダメージは+N」(被ダメ増加, §B-24-5) と排他になる。
  ...
  //     コノヨザル / オーダイル / ドンファン
  {
    tags: ['ダメージ操作', 'ダメージ操作>与ダメ強化'],
    condition: (all) =>
      /相手の.{0,40}へのダメージは.{0,5}「[+＋]\d+」/.test(all),
  },
  // §B-24-5  被ダメ増加: 「このワザを受けたポケモンが受けるワザのダメージは「+N」」
  //   ターゲットに脆弱性マーカーを付与し、次の自分の番の与ダメを底上げするバフ
  //   — ビブラーバ / マッギョ
  {
    tags: ['ダメージ操作', 'ダメージ操作>被ダメ増加'],
    condition: (all) =>
...
// ── §B-23  〜のポケモン ──────────────────────────────────────────────────
// Cards whose name starts with <キャラクター名>の (main-series trainers only).
// Excludes ロケット団の (handled above under §B-18), non-character prefixes
// (e.g. ○○のみ berries, regional Pikachu place names, generic nouns),
// and non-series entries such as インフルエンサー.
{
...
const GROUP_ORDER = [
  ...
  'サーチ', 'サーチ>ポケモン', 'サーチ>サポート', 'サーチ>グッズ', 'サーチ>自由選択',
  'トラッシュ回収', 'トラッシュ回収>ポケモン', 'トラッシュ回収>エネルギー', 'トラッシュ回収>自由選択',
  '特殊状態', '特殊状態>どく', '特殊状態>やけど', '特殊状態>こんらん', '特殊状態>ねむり', '特殊状態>マヒ',
  '回復', '回復>固定HP回復', '回復>全回復', '回復>複数回復',
  'ダメカン操作',
  'エネ移動', 'エネ移動>自分・場のエネ自由配分', 'エネ移動>自分・集約', 'エネ移動>自分・拡散', 'エネ移動>相手のエネ',
  'エネ除去', 'エネ除去>相手エネ除去', 'エネ除去>自己コスト',
  'ワザロック', 'ワザロック>自己ロック', 'ワザロック>相手ロック',
  'にげロック', 'にげロック>相手', 'にげロック>自分',
  ...
  '進化加速', '進化加速>ルール制限解除', '進化加速>山札から', '進化加速>条件進化',
  '山札破壊',
  'スタジアム', 'スタジアム>破壊', 'スタジアム>参照', 'スタジアム>封印',
  'サイド操作',
  ...
  // §B-24 ダメージ操作
  'ダメージ操作', 'ダメージ操作>抵抗無視', 'ダメージ操作>弱点抵抗無視', 'ダメージ操作>弱点無視', 'ダメージ操作>与ダメ強化', 'ダメージ操作>被ダメ増加',
...
const SECTION_MAP = {
  ...
  'ダメカン操作':       '§B-4','エネ移動':     '§B-5',
  'エネ移動>自分・場のエネ自由配分': '§B-5','エネ移動>自分・集約': '§B-5','エネ移動>自分・拡散': '§B-5','エネ移動>相手のエネ': '§B-5',
  'エネ除去': '§B-6',
  'ワザロック':         '§B-7','にげロック': '§B-27','にげロック>相手': '§B-27','にげロック>自分': '§B-27','グッズサポートロック': '§B-8','軽減': '§B-9',
  'カウンター効果':     '§B-10','自傷':        '§B-11','自傷>ダメカン': '§B-11','自傷>ダメージ': '§B-11','ベンチ展開': '§B-12',
  '進化加速':           '§B-13','進化加速>ルール制限解除': '§B-13','進化加速>山札から': '§B-13','進化加速>条件進化': '§B-13','山札破壊':    '§B-14','スタジアム': '§B-15',
  'サイド操作':         '§B-16','条件ダメージ':'§B-17','特殊連動': '§B-18',
  'ACE SPEC':           '§B-19','ポケモンチェック': '§B-20',
  '特性無効':           '§B-22',
  '〜のポケモン':       '§B-23',
