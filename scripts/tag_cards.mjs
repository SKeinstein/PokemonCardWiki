...
const TAG_RULES = [
  ...
  // ── §B-7  ワザロック(相手)（旧名: ロック付与・Phase 32-2）──────────────────
  // >ワザ: ポケモンのワザによる相手ワザ封じ
  {
    tags: ['ワザロック(相手)', 'ワザロック(相手)>ワザ'],
    condition: (all) => /次の相手の番.{1,20}ワザが使えない/.test(all),
  },
  // >スタジアム: 相手のスタジアム設置を封じる（ダイオウドウ/イーユイ）
  {
    tags: ['ワザロック(相手)', 'ワザロック(相手)>スタジアム'],
    condition: (all) => /相手は手札からスタジアムを出せない/.test(all),
  },
  // >どうぐ: 相手のポケモンのどうぐ効果を無効化（ジャミングタワー）
  {
    tags: ['ワザロック(相手)', 'ワザロック(相手)>どうぐ'],
    condition: (all) => /「?ポケモンのどうぐ」?.{0,30}効果は.{0,20}なくなる/.test(all),
  },
  // ── §B-27  にげられない（旧: 逃げる操作>相手ロック・Phase 32-2）──────────────
  // >ワザ: ポケモンのワザによる相手にげられない
  {
    tags: ['にげられない', 'にげられない>ワザ'],
  ...
  // §B-27  にげられない（自分）（旧: 逃げる操作>自分ロック・Phase 32-2）
  {
    tags: ['にげられない（自分）'],
  ...
  // 特殊状態>耐性 is handled by the general pattern rule in §B-2 above
...
const GROUP_ORDER = [
  ...
  '特殊状態', '特殊状態>どく', '特殊状態>やけど', '特殊状態>こんらん', '特殊状態>ねむり', '特殊状態>マヒ', '特殊状態>耐性', '特殊状態>参照',
  '回復', '回復>固定HP回復', '回復>全回復', '回復>特殊状態',
  '最大HP強化',
  'ダメカン操作', 'ダメカン操作>のせる', 'ダメカン操作>移動', 'ダメカン操作>反射',
  'エネ除去', 'エネ除去>相手エネ除去', 'エネ除去>自己コスト', 'エネ除去>つけかえ',
  'ワザロック(相手)', 'ワザロック(相手)>ワザ', 'ワザロック(相手)>スタジアム', 'ワザロック(相手)>どうぐ',
  '逃げる操作', '逃げる操作>コスト無償',
  'にげられない', 'にげられない>ワザ', 'にげられない（自分）',
...
const SECTION_MAP = {
  ...
  'エネ除去': '§B-6',
  'ワザロック(相手)': '§B-7','ワザロック(相手)>ワザ': '§B-7','ワザロック(相手)>スタジアム': '§B-7','ワザロック(相手)>どうぐ': '§B-7',
  '逃げる操作': '§B-27','逃げる操作>コスト無償': '§B-27',
  'にげられない': '§B-27','にげられない>ワザ': '§B-27','にげられない（自分）': '§B-27',
  'ロック': '§B-8','ロック>トレーナー効果無効': '§B-8','ロック>特性': '§B-8','軽減': '§B-9',
