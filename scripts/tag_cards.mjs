...
const TAG_RULES = [
  ...
  // §1-3  にげるコスト>増加（テキスト）
  {
    ...
    condition: (all) => /にげるためのエネルギー.{1,30}多くなる/.test(all),
  },
...
const GROUP_ORDER = [
  'ポケモン入れ替え', 'ポケモン入れ替え>ワザ', 'ポケモン入れ替え>特性', 'ポケモン入れ替え>にげるコスト', 'ポケモン入れ替え>にげるコスト>軽減', 'ポケモン入れ替え>にげるコスト>増加', 'ポケモン入れ替え>グッズ', 'ポケモン入れ替え>スタジアム',
