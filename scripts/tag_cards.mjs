...
const TAG_RULES = [
  ...
  // ── §B-1  ドロー・サーチ ──────────────────────────────────────────────────
  // §B-1-1  固定枚数ドロー
  // 「手札がN枚になるように、山札を引く」(手札補充型) も同カテゴリとして捕捉
  // — ハピナスex / シロナのガブリアスex / ネオラント / タブンネ / デオキシス(サイコスピード) 等
  {
    ...
    condition: (all) =>
      ...
      /手札が\d+枚になるように.{0,20}山札を引く/.test(all),
  },
  // §B-1-2  ポケモンサーチ（山札→手札）
  // NOTE: exclude 「ポケモンのどうぐ」 (Pokémon Tool = Trainer) from ポケモンサーチ
  {
    ...
    condition: (all, atk, abl, name) =>
      (/山札[からのに].{0,10}ポケモン.{1,25}手札に加える/.test(all) && !/ポケモンのどうぐ/.test(all)) ||
  ...
  // §B-1-3  トレーナーズサーチ
  {
    ...
    condition: (all) =>
      ...
      /山札を上から.{1,15}枚見.{1,80}サポ.{0,5}を.{1,40}手札/.test(all),
  },
  // §B-1-4  トラッシュ回収（手札へ）
  {
    ...
    condition: (all) =>
      ...
      /トラッシュ.{1,10}基本エネルギー.{1,40}手札に加える/.test(all),
  },
  ...
  // §B-9-6  無効>全体: coin-flip or unconditional complete negation (active, not type/bench-specific)
...
const GROUP_ORDER = [
  ...
  '〜のポケモン>探検家',
  // §B-26 ベンチ保護
  '無効', '無効>ベンチへのワザダメージ', '無効>ベンチへの効果とダメージ', '無効>ワザの効果のみ', '無効>ダメカン配置', '無効>全体', '無効>条件付きその他',
  // §B-24 ダメージ操作
  'ダメージ操作', 'ダメージ操作>抵抗無視',
...
const SECTION_MAP = {
  ...
  'ダメカン操作':       '§B-4','エネ移動':     '§B-5','エネ移動>自分のみ': '§B-5','エネ除去': '§B-6',
