...
const TAG_RULES = [
  ...
  //   — チェレン / レントラー / ハチクマンの研究 / リーリエ 等
  {
    ...
    condition: (all) => {
      ...
      if (!/山札を\d+枚引く|山札から\d+枚引く|山札を.{1,5}枚引く|\d+枚、?自分の山札を引く|の数ぶん、?自分の山札を引く|山札を\d+枚引いてよい/.test(all)) return false;
  ...
  //   「きぜつさせたなら/させたとき」(条件・トリガー) は末尾「。」要求で除外
  {
    ...
    condition: (all, atk) =>
      ...
      // ロケット団のベトベター: トラッシュ送り (きぜつ扱いでないがサイド外除去として同分類)
      /番の終わりに、?このワザを受けたポケモンと、?ついているすべてのカードを、?トラッシュする/.test(atk),
  },
  ...
  // ワザコピー（C-18）
  // ジーランス「進化前に持っていたワザを、すべて使える」/ コアメモリ「書かれているワザを使える」(2026-06-12)
  {
    tags: ['ワザコピー'],
    condition: (all) =>
      /このワザの効果として使う|このワザとして使う/.test(all) ||
      /持っていたワザを.{0,5}すべて使える|書かれているワザを使える/.test(all),
  },
  // 退化（C-13）
...
const SECTION_MAP = {
  ...
  '山札操作': '§P33-O', 'エネ価値増加': '§P33-O', 'エネコスト軽減': '§P33-O',
